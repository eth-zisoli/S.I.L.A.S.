...
**🗣️ A third person in the room** - Unlike voice assistants that only respond to rigid commands, Silas understands conversations. It maintains a short temporary rolling context of what's being discussed, so when you ask "Silas, what do you think?" it knows exactly what you're talking about. Have it chime into discussions with friends, help debug code while you talk through problems, or weigh in on decisions.
...
👤 Alice: Silas, what do you think?
  📝 Heard: "What do you think Silas?"
...
```
Silas understood the entire conversation and gave a contextual answer — no need to repeat the question.

**Browser automation via MCP:**
```
  📝 Heard: "Open YouTube Silas."
...
**Health-aware advice:**
```
  📝 Heard: "Should I order pizza tonight considering my health goals Silas?"
...
```
  📝 Heard: "What are some news from today that might interest me Silas?"
...
**Natural wake word placement** (say "Silas" anywhere):
```
  📝 Heard: "Give me a random topic to discuss Silas."
...
**Echo detection** (Silas ignores its own speech):
```
🤖 Silas
I have opened YouTube for you.
...
Get the latest from [GitHub Releases](https://github.com/isair/silas/releases):

| Platform | Download | Run |
|----------|----------|-----|
| **Windows** | `Silas-Windows-x64.zip` | Extract → Run `Silas.exe` |
| **macOS** | `Silas-macOS-arm64.zip` | Extract → Move to Applications → Right-click → Open |
| **Linux** | `Silas-Linux-x64.tar.gz` | `tar -xzf` → Run `./Silas/Silas` |
...
- **Conversational Awareness** - Understands ongoing discussions. Ask "Silas, what do you think?" and it knows what you're talking about. Works naturally in multi-person conversations.
...
- **Natural Voice** - Say "Silas" anywhere in your sentence, interrupt with "stop", follow up without repeating the wake word
...
If you have an NVIDIA GPU, Silas can use CUDA for much faster speech recognition. The Windows installer offers an optional CUDA download during setup. For development:
```bash
...
Small chat models (~2B, e.g. `gemma4:e2b`) degrade sharply as their prompt grows. Silas runs two cheap distil passes to keep the prompt tight:
...
Both digest passes auto-enable for small models (≤7B) and stay off for large models. For small models, tool-result digest also prevents large fetch_web_page payloads from blowing the context window. Override in `~/.config/silas/config.json`:
...
```json
{
  "tts_piper_model_path": "~/.local/share/silas/models/piper/en_GB-alan-medium.onnx"
}
```
...
Silas can provide location-aware responses (weather, local time, etc.) using a local GeoLite2 database — no cloud geolocation services are used.
...
If your ISP uses carrier-grade NAT (CGNAT), Silas automatically resolves your true public IP via the same OpenDNS DNS query. This can be disabled:
...
**Setup:** Register for a free [MaxMind GeoLite2](https://www.maxmind.com/en/geolite2/signup) account, download the City database (MMDB format), and save it to `~/.local/share/silas/geoip/GeoLite2-City.mmdb`. The setup wizard will guide you through this.
...
Connect Silas to external tools via [MCP servers](https://github.com/topics/mcp-server):
...
"Silas, turn on the living room lights" / "set bedroom to 72°" / "run good night scene"
...
**First startup takes a bit** - Silas pre-warms the Whisper, chat, and intent-judge models before announcing "Listening!" so the first engagement feels instant. This adds a few seconds on cold start and is bounded at 60 s — if Ollama is slow, Silas will start listening anyway and load the models on demand.

**Silas doesn't hear me** - Check microphone permissions, speak clearly after "Silas"
...
```bash
git clone https://github.com/isair/silas.git
cd silas
...
Verify: `sudo lsof -i -n -P | grep silas` (should only show 127.0.0.1 to Ollama)
...
When DuckDuckGo is rate-limited or returns nothing fetchable, Silas walks
...
- **Local storage** - Everything in `~/.local/share/silas`
...
[Report issues](https://github.com/isair/silas/issues) · [Discussions](https://github.com/isair/silas/discussions) · [Sponsor](https://github.com/sponsors/isair)
