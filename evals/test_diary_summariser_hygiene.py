...
class TestDiarySummariserHygieneLive:
    ...
    def _summarise(self, chunks: list[str]) -> tuple[str, str]:
        from silas.memory.conversation import generate_conversation_summary
    ...
    def test_unrelated_topics_are_not_welded_into_one_clause(self):
        """Regression for the Possessor/Silas field incident.

        Two distinct topics (the 2020 Cronenberg film Possessor, and the
        MCU AI character named Silas) in the same conversation must not
        be summarised as a single welded clause like "the movie Possessor
        and the character Silas, identified as the MCU AI...". Downstream
        ...
        chunks = [
            ...
            "User: No, unrelated — why are you called Silas?",
            "Assistant: My name is a nod to the MCU character Silas, the AI created by Tony Stark and later embodied by Vision.",
        ]
        summary, _ = self._summarise(chunks)
        print(f"\n  Summary: {summary}")
        ...
        for s in sentences:
            ...
            mentions_mcu_silas = any(t in low for t in mcu_tokens)
            if mentions_possessor and mentions_mcu_silas:
                welded.append(s)

        if welded:
            pytest.xfail(
                f"Small judge model {JUDGE_MODEL} welded Possessor with MCU-Silas "
        ...
        assert "silas" in lowered, f"Silas topic dropped: {summary}"
