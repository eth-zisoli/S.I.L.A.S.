...
class TestTopicSwitching:
    ...
    def test_weather_then_store_hours(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch('silas.reply.engine.run_tool_with_retries', side_effect=mock_tool_run), \
             patch('silas.reply.engine.get_location_context_with_timezone', return_value=("Location: Kensington, Royal Kensington and Chelsea, United Kingdom", None)):
    ...
    def test_search_then_weather(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch('silas.reply.engine.run_tool_with_retries', side_effect=mock_tool_run), \
             patch('silas.reply.engine.get_location_context_with_timezone', return_value=("Location: Kensington, UK", None)):
