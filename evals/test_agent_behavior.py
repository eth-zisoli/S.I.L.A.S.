...
class TestContextUtilization:
    ...
    def test_location_context_in_search(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        from silas.reply.engine import run_reply_engine
        ...
        with patch('silas.reply.engine.run_tool_with_retries', side_effect=mock_tool_run), \
             patch('silas.reply.engine.chat_with_messages', side_effect=mock_chat), \
             patch('silas.reply.engine.get_location_context_with_timezone', return_value=(f"Location: {user_location}", None)), \
             patch('silas.reply.engine.extract_search_params_for_memory', return_value={"keywords": []}):
...
class TestMultiStepReasoning:
    ...
    def test_nutrition_advice_uses_memory_and_data(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch('silas.reply.engine.run_tool_with_retries', side_effect=mock_tool_run), \
             patch('silas.reply.engine.chat_with_messages', side_effect=mock_chat), \
             patch('silas.reply.engine.extract_search_params_for_memory', return_value={"keywords": ["health", "diet"]}):
...
class TestMemoryEnrichment:
    ...
    def test_enrichment_extracts_correct_keywords(self, query: str, expected_keywords: list, mock_config):
        ...
        from silas.reply.enrichment import extract_search_params_for_memory
    ...
    def test_enrichment_provides_context_to_llm(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch('silas.reply.engine.chat_with_messages', side_effect=mock_chat), \
             patch('silas.reply.engine.extract_search_params_for_memory', return_value={"keywords": ["dinner", "food", "preferences"]}), \
             patch('silas.memory.conversation.search_conversation_memory_by_keywords', return_value=mock_memory_results):
    ...
    def test_llm_uses_enrichment_for_personalised_queries(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch('silas.reply.engine.run_tool_with_retries', side_effect=mock_tool_run), \
             patch('silas.reply.engine.chat_with_messages', side_effect=mock_chat), \
             patch('silas.reply.engine.extract_search_params_for_memory', return_value={"keywords": ["interests", "hobbies", "preferences"]}), \
             patch('silas.memory.conversation.search_conversation_memory_by_keywords', return_value=mock_enrichment_context):
...
class TestLiveEndToEnd:
    ...
    def test_weather_query_live(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch('silas.reply.engine.get_location_context_with_timezone', side_effect=mock_get_location):
    ...
    def test_personalized_query_recalls_memory_live(self, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch('silas.reply.engine.run_tool_with_retries', side_effect=mock_tool_run), \
             patch('silas.reply.engine.get_location_context_with_timezone', return_value=("Location: London, UK", None)), \
             patch('silas.memory.conversation.search_conversation_memory_by_keywords', return_value=mock_enrichment_context):
    ...
    @pytest.mark.parametrize("query", [
        pytest.param(
            "Recall my interests, then search the web for news on them, Silas.",
            id="explicit-recall-then-search",
        ),
        pytest.param(
            "Search the web for news that would interest me, Silas.",
            id="news-that-would-interest-me",
        ),
        pytest.param(
            "Find me news of interest to me, Silas.",
            id="news-of-interest-to-me",
        ),
        pytest.param(
            "What news today is interesting for me, Silas?",
    ...
    def test_interest_flavoured_query_live(self, query, mock_config, eval_db, eval_dialogue_memory):
        ...
        and search the web for news on them, Silas." The intent judge paraphrased
...
class TestHelpfulness:
    ...
    def test_no_deflection_for_weather_forecast_live(
        ...
        with patch('silas.reply.engine.run_tool_with_retries', side_effect=mock_tool_run), \
             patch('silas.reply.engine.get_location_context_with_timezone', return_value=("Location: Tbilisi, Georgia", None)):
    ...
    def test_tool_retry_after_failure_live(
        ...
        """
        from silas.reply.engine import run_reply_engine
        from silas.reply.prompts import detect_model_size, ModelSize
        from helpers import JUDGE_MODEL
        ...
        def mock_tool_run(db, cfg, tool_name, tool_args, **kwargs):
            ...
            from silas.tools.types import ToolExecutionResult
    ...
    def test_graph_knowledge_surfaced_in_reply_live(
        ...
        nodes = [
            ...
            _Node(
                ...
                "primarily on a local voice-assistant side-project called Silas.",
            ),
        ]
        ...
        with patch("silas.reply.engine.extract_search_params_for_memory", return_value=fake_extract), \
             patch("silas.memory.graph.GraphMemoryStore", _FakeStore), \
             patch("silas.memory.conversation.search_conversation_memory_by_keywords", return_value=[]), \
             patch("silas.reply.engine.get_location_context_with_timezone",
        ...
        fact_keywords = [
            ...
            "equals money", "software engineer", "silas",
        ]
    ...
    def test_does_not_deny_long_term_memory_live(
        ...
        Silas *does* have persistent memory (the knowledge graph + diary), so
        ...
        with patch("silas.reply.engine.extract_search_params_for_memory",
