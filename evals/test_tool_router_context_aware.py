...
Motivating field incident (2026-04-20):
  User asked "what time is it, Silas?". The router, having no view of the
...
def _route(query: str, context_hint):
    ...
    from silas.tools.registry import BUILTIN_TOOLS
    from silas.tools.selection import select_tools, ToolSelectionStrategy
...
class TestRouterReturnsNoneWhenContextAnswers:
    ...
    def test_time_query_with_time_in_context_returns_none(self):
        selected = _route("what time is it, Silas?", _TIME_LOCATION_HINT)
