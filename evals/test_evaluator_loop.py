...
class TestPrematureProseNudge:
    ...
    def test_navigate_prose_gets_nudged_into_tool_call(
        ...
        with patch("silas.reply.engine.select_tools", side_effect=router), \
             patch("silas.reply.engine.run_tool_with_retries", side_effect=runner), \
             patch(
                 "silas.reply.engine.get_location_context_with_timezone",
...
class TestMaxTurnDigestCaveat:
    ...
    def test_max_turn_triggers_digest(
        ...
        with patch("silas.reply.engine.select_tools", side_effect=router), \
             ...
             patch("silas.reply.engine.chat_with_messages", side_effect=_always_tool_call), \
             patch("silas.reply.engine.digest_loop_for_max_turns", side_effect=_spy_digest):
            reply = run_reply_engine(
                db=eval_db, cfg=mock_config, tts=None,
...
class TestStructuredToolCallEmission:
    ...
    def test_evaluator_emits_structured_tool_call_for_obvious_search(
        ...
        from silas.reply.evaluator import evaluate_turn
