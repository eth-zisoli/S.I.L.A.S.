...
Captured from a real field session on 2026-04-20 where gemma4:e2b:
  1. First session (before wake-word fix): model replied with a pure greeting
     because the trailing vocative "Silas" triggered GREETING HANDLING.
...
def _patch_graph_enrichment():
    ...
    def _cm():
        ...
        with patch(
            "silas.memory.graph.GraphMemoryStore.search_nodes",
            return_value=nodes,
        ), patch(
            "silas.memory.graph.GraphMemoryStore.get_ancestors",
...
POISONED_SUMMARY = (
    ...
    'The discussion then shifted to the character "Silas," identified as the '
    ...
    'character. (Topics: Possessor, movie, Silas, AI character, Marvel Cinematic Universe)'
)
...
class TestPossessorFieldRepro:
    ...
    def _run(self, query: str, mock_config, eval_db, eval_dialogue_memory):
        ...
        with patch(
            'silas.memory.conversation.search_conversation_memory_by_keywords',
            return_value=[POISONED_SUMMARY],
        ), patch(
            'silas.reply.engine.run_tool_with_retries',
