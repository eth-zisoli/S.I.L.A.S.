...
def _generate_thinking_pad_samples() -> tuple[np.ndarray, int]:
    ...
    Designed to run indefinitely while Silas thinks. Two tricks make
...
class TunePlayer:
    """Plays a thinking-pad tune in a loop while Silas is processing.
    ...
    def _play_fallback_tune(self) -> None:
        ...
        while not self._stop_event.is_set():
            try:
                print(f"\r[silas] {patterns[i % len(patterns)]} processing...",
                      end="", flush=True)
                time.sleep(0.2)
                i += 1
            except Exception:
