...
- Example: "I made carbonara" + "Silas find recipe for that" -> "find recipe for carbonara"
- Example: "the weather will be nice tomorrow" + "Silas what do you think" -> "what do you think about the weather tomorrow"
- Example: "the new iPhone is cool" + "Silas how much does it cost" -> "how much does the iPhone cost"
- Example: "the AirPods sound great" + "Silas how much do they cost" -> "how much do the AirPods cost". NOT "how much do they cost" — pronoun MUST be replaced with the named topic in the output query even if you resolved it correctly in your reasoning.
- Example: "did you catch the ball game" + "the new iPhone is out" + "I want the pro model" + "Silas what's the price" -> "what's the price of the iPhone pro model". NOT "what's the price of the pro model" (which pro model? ambiguous) — always prepend the brand/parent from earlier segments.
...
Examples:
- "Silas what time is it" -> {{"directed": true, "query": "what time is it", "stop": false, "confidence": "high", "reasoning": "wake word + question"}}
- "what do you know about the movie called Possessor Silas" -> {{"directed": true, "query": "what do you know about the movie called Possessor", "stop": false, "confidence": "high", "reasoning": "wake word at end; entity is Possessor, not Possessor Silas"}}
- "I just ate a big Mac Silas" -> {{"directed": true, "query": "I just ate a big Mac", "stop": false, "confidence": "high", "reasoning": "wake word at end; 'Mac' is part of the brand name 'Big Mac', not a compound surname with Silas"}}
- "hey Silas what's the weather in London" -> {{"directed": true, "query": "what's the weather in London", "stop": false, "confidence": "high", "reasoning": "wake word removed from mid-sentence position"}}
- "Silas say something please" -> {{"directed": true, "query": "say something please", "stop": false, "confidence": "high", "reasoning": "self-contained imperative"}}
- "Silas tell me a joke" -> {{"directed": true, "query": "tell me a joke", "stop": false, "confidence": "high", "reasoning": "self-contained imperative"}}
- Previous "dinosaurs are cool" + Current "Silas what do you think about that" -> {{"directed": true, "query": "what do you think about dinosaurs being cool", "stop": false, "confidence": "high", "reasoning": "resolved 'that' to dinosaurs"}}
- Previous "How's the weather?" + Current "Silas answer that" -> {{"directed": true, "query": "how is the weather", "stop": false, "confidence": "high", "reasoning": "imperative -> re-issue prior question"}}
- Previous "How tall is Mount Everest" + Noise "some unrelated chatter" + Current "Silas answer that" -> {{"directed": true, "query": "how tall is Mount Everest", "stop": false, "confidence": "high", "reasoning": "imperative -> re-issue prior QUESTION; ignore the chatter segment, re-issue the original question even when noise sits between"}}
- Previous "What's the capital of Portugal" + Current "Silas go ahead and answer" -> {{"directed": true, "query": "what is the capital of Portugal", "stop": false, "confidence": "high", "reasoning": "multi-word imperative ('go ahead and answer') is the same pattern as 'answer that' -> re-issue prior question; do NOT pass the imperative through literally"}}
...
def create_intent_judge(cfg) -> Optional[IntentJudge]:
    """Create an intent judge from Silas configuration.
    ...
    config = IntentJudgeConfig(
        assistant_name=str(getattr(cfg, "wake_word", "silas")).capitalize(),
