...
def _print_cuda_unavailable_hint(missing_libs: list[str]) -> None:
    ...
    print(
        "  💡 For GPU acceleration, click 'Reinstall GPU libraries' in the Silas tray menu",
        flush=True,
    )
...
class VoiceListener(threading.Thread):
    ...
    def _set_face_state_listening(self) -> None:
        ...
        try:
            from desktop_app.face_widget import get_silas_state, SilasState
            get_silas_state().set_state(SilasState.LISTENING)
        except ImportError:
            pass
        except Exception as e:
    ...
    def _process_transcript(self, text: str, utterance_energy: float = 0.0, utterance_start_time: float = 0.0, utterance_end_time: float = 0.0) -> None:
        ...
        if not received_during_tts and not self._is_thinking_tune_active():
            ...
            if in_hot_window:
                ...
                wake_word = getattr(self.cfg, "wake_word", "silas")
            ...
            if intent_judgment is not None:
                ...
                if intent_judgment.directed and intent_judgment.query:
                    ...
                    if not could_be_hot_window:
                        ...
                        # The mic can pick up Silas's own TTS output and Whisper
    ...
    def _dispatch_query(self, query: str) -> None:
        ...
        try:
            from desktop_app.face_widget import get_silas_state, SilasState
            state_manager = get_silas_state()
            state_manager.set_state(SilasState.THINKING)
    ...
    def run(self) -> None:
            ...
            try:
                ...
                if check_thread.is_alive():
                    ...
                    if "unapproved" in error_str or "denied" in error_str or "access" in error_str or "-9999" in str(e):
                        ...
                        print("  │                                                         │", flush=True)
                        print("  │  Then restart Silas.                                  │", flush=True)
                        print("  └─────────────────────────────────────────────────────────┘", flush=True)
                        print("", flush=True)
                    return
                elif mic_ok.is_set():
        ...
        with stream:
            ...
            wake_word = getattr(self.cfg, "wake_word", "silas").lower()
            ...
            try:
                from desktop_app.face_widget import get_silas_state, SilasState
                state_manager = get_silas_state()
                state_manager.set_state(SilasState.IDLE)
            except Exception:
                pass
