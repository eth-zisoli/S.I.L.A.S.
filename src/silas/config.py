...
def _default_db_path() -> str:
    base = Path.home() / ".local" / "share" / "silas"
    base.mkdir(parents=True, exist_ok=True)
    return str(base / "silas.db")
...
def default_config_path() -> Path:
    ...
    if xdg:
        return Path(xdg) / "silas" / "config.json"
    return Path.home() / ".config" / "silas" / "config.json"
...
def load_config() -> Dict[str, Any]:
    ...
    """
    cfg_path_env = os.environ.get("SILAS_CONFIG_PATH")
...
def get_default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Wake Word Detection
        "wake_word": "silas",
        "wake_aliases": ["silus", "sila", "silar", "silash", "sirus", "cyrus", "sailas", "silaas", "silos", "si less"],
        ...
        # Task-list planner (see src/silas/reply/planner.spec.md). Empty
        ...
        # MCP Integration (external servers Silas can use). No defaults.
        "mcps": {},
    }
...
def export_example_config(include_db_path: bool = False) -> Dict[str, Any]:
    ...
    if not include_db_path:
        ...
        config["db_path"] = "~/.local/share/silas/silas.db"
    return config
...
def load_settings() -> Settings:
    ...
    # Env overrides: SILAS_VOICE_DEBUG, SILAS_WHISPER_BACKEND
    voice_debug = os.environ.get("SILAS_VOICE_DEBUG", "0") == "1"
    ...
    wake_word = str(merged.get("wake_word", "silas")).strip().lower()
    ...
    whisper_backend = os.environ.get("SILAS_WHISPER_BACKEND", "").lower() or str(merged.get("whisper_backend", "auto")).lower()
