...
Gated on the env var `SILAS_DUMP_PROMPTS=1` — off by default because the
dumps contain the full system prompt, memory digest and tool output (likely
PII). Users opt in only when hunting a bug.

Files are written to `~/.local/share/silas/prompts/` as per-turn JSON so
...
def _dump_dir() -> Path:
    base = Path.home() / ".local" / "share" / "silas" / "prompts"
