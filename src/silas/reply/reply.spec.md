...
- Components:
  - Reply Engine (`src/silas/reply/engine.py`): Orchestrates conversation-memory enrichment, tool-use protocol, messages loop, output, and memory update.
  - System Prompt (`src/silas/system_prompt.py`): Provides a unified `SYSTEM_PROMPT` with adaptive guidance for all topics. Declares the assistant's persona — a British butler named Silas with dry wit and light, good-natured sarcasm — with explicit behavioural rules (answer-first/quip-second, at most one quip, skip the quip for serious topics, no butler clichés, sarcasm never aimed at the user). The rules are phrased concretely rather than as tone adjectives so small models can follow them. Persona behaviour is not currently covered by an eval; add one if the tone regresses or the rules evolve.
  - LLM Gateway (`src/silas/llm.py`): `chat_with_messages` sends the messages array and returns raw JSON; `extract_text_from_response` normalizes content across providers.
  - Conversation Memory (`src/silas/memory/conversation.py`): Supplies recent dialogue messages and keyword/time-bounded recall.
  - Enrichment LLM (`src/silas/reply/enrichment.py`): Extracts search params (keywords and optional time bounds) from the current query to drive conversation recall.
...
2. Recent Dialogue Context
   ...
   - A **recall gate** (`src/silas/memory/recall_gate.py`, deterministic, no LLM) skips diary / graph / memory-digest enrichment when the hot window already covers the topic (≥50% content-word overlap with a fresh tool-result row). Language-agnostic via `\w{3,}` with `re.UNICODE`. Fail-open on any error. The gate is bypassed when the planner explicitly emitted a `searchMemory` step, planner intent always wins over coverage heuristics. See `src/silas/memory/recall_gate.spec.md`.
...
3. Pre-flight Planner
   - The task-list planner (`plan_query` in `src/silas/reply/planner.py`) runs **first**, before any memory lookup or tool routing. It sees the query, a compact dialogue snippet, and the full builtin + MCP tool catalogue (names + one-line descriptions).
...
6. Agentic Messages Loop with Dynamic Context
   ...
   - `toolSearchTool` is a builtin; see `src/silas/tools/builtin/tool_search.spec.md`.
...
**Detection:**
```python
from silas.reply.prompts import detect_model_size, get_system_prompts
...
See `src/silas/reply/prompts/prompts.spec.md` for full prompt architecture documentation.
...
To mitigate both, `digest_memory_for_query` (in `src/silas/reply/enrichment.py`) runs a cheap LLM pass over the raw diary + graph block and produces a short relevance-filtered note that replaces both `conversation_context` and `graph_context` in the reply system prompt.
...
`digest_tool_result_for_query` (in `src/silas/reply/enrichment.py`) runs a cheap LLM pass over the raw tool output and returns an attributed fact note that replaces the tool-role message content before it reaches the main model.
