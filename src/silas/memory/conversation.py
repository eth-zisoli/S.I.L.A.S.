...
def is_tool_message(msg: dict) -> bool:
    ...
    Covers both protocols Silas speaks:
    ...
    - Text-tool fallback (small models): the tool result is appended as a
      ...
      done by the reply engine in `src/silas/reply/engine.py` (see the
...
8. CRITICAL topic-separation rule — do NOT weld unrelated topics into one grammatical clause. If the conversation covered two distinct subjects (e.g. a film and a person, a recipe and a weather query, two different named entities), write a separate sentence for each, each with its own subject and verb. A welded clause reads to downstream retrievers — and to other LLMs enriching future replies — as a single claim about both referents, and silently corrupts the record.
   ...
   Example — two distinct topics raised in the same conversation (a film, and the name "Silas" meaning the MCU character). The BAD version welded them so downstream readers treated the MCU description as pertaining to the film:
     BAD: "The conversation focused on the movie Possessor and the character Silas, identified as the artificial intelligence from the Marvel Cinematic Universe, created by Tony Stark and later embodied by Vision."
     GOOD: "The user asked about the movie Possessor; the assistant said it is a 2020 science-fiction horror film directed by Brandon Cronenberg. Separately, the user asked about the name Silas; the assistant said the MCU character Silas is an AI created by Tony Stark and later embodied by Vision."
