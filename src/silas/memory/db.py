...
class Database:
    def __init__(self, db_path: str, sqlite_vss_path: Optional[str] = None) -> None:
        ...
        if not self.is_vss_enabled:
            ...
            if store_type == "FAISSVectorStore":
                debug_log("Using FAISS vector store for fast search", "silas")
            else:
                debug_log("Using Python fallback vector store", "silas")
    ...
    def get_conversation_summary(self, date_utc: str, source_app: str = "silas") -> Optional[sqlite3.Row]:
