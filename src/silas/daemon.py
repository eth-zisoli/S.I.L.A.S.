...
def main() -> None:
    ...
    debug_log("daemon started", "silas")
        ...
        try:
            ...
            def _on_dictation_start():
                ...
                try:
                    ...
                    get_silas_state().set_state(SilasState.DICTATING)
                except Exception:
                    pass
            ...
            def _on_dictation_processing_start():
                try:
                    from desktop_app.face_widget import SilasState, get_silas_state
                    get_silas_state().set_state(SilasState.DICTATION_PROCESSING)
                except Exception:
                    pass
            ...
            def _on_dictation_end():
                ...
                try:
                    from desktop_app.face_widget import SilasState, get_silas_state
                    get_silas_state().set_state(SilasState.IDLE)
                except Exception:
                    pass
        ...
        try:
            ...
            while True:
                ...
                if not line:  # EOF - stdin closed
                    debug_log("stdin closed, requesting stop", "silas")
                    _global_stop_requested = True
                    break
                line = line.strip()
                if line == "SHUTDOWN":
                    debug_log("SHUTDOWN command received, requesting stop", "silas")
                    _global_stop_requested = True
                    break
        except Exception:
    ...
    try:
        ...
        debug_log("daemon received KeyboardInterrupt", "silas")
    finally:
        print("🔄 Daemon shutting down - saving memory...", flush=True)
        debug_log("daemon finally block starting - performing cleanup", "silas")

        # Clean shutdown - stop dictation first
        if dictation is not None:
            debug_log("stopping dictation engine...", "silas")
            dictation.stop()
            debug_log("dictation engine stopped", "silas")

        if voice_thread is not None:
            debug_log("stopping voice thread...", "silas")
            voice_thread.stop()
            try:
                voice_thread.join(timeout=2.0)
            except Exception:
                pass
            debug_log("voice thread stopped", "silas")

        # Final diary update before shutdown
        debug_log("performing final diary update (force=True)...", "silas")
        ...
        debug_log("diary update complete", "silas")
        ...
        try:
            ...
            debug_log(f"MCP runtime shutdown error: {_e}", "silas")
        ...
        debug_log("daemon stopped", "silas")
