...
|-------|-----|-------|-----------|-----|
| Location | Final resolved IP | Location info dict | `~/.local/share/silas/location_cache.json` | `location_cache_minutes` (default 60 min) |
| CGNAT resolution | Original CGNAT IP | `(timestamp, resolved_ip \| None)` | `~/.local/share/silas/cgnat_cache.json` | 1 hour (hardcoded) |
...
- Expected path: `~/.local/share/silas/geoip/GeoLite2-City.mmdb`
...
- **Daemon** (`src/silas/daemon.py`): Calls `get_location_context` at startup using config values.
- **Reply Engine** (`src/silas/reply/engine.py`): Refreshes location context each agentic turn via `get_location_context`.
