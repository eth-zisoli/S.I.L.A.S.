...
First-run wizard that ensures Ollama, required models, and Whisper are ready before Silas starts.

## Overview

The setup wizard is shown only when **user action is required** — it is not shown merely because the Ollama server isn't running (Silas can auto-start it). The two triggers are:
