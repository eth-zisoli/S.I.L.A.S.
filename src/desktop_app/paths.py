...
def get_log_dir() -> Path:
    """Return the platform-appropriate directory for Silas logs.
    ...
    if sys.platform == "darwin":
        preferred = Path.home() / "Library" / "Logs" / "Silas"
    elif sys.platform == "win32":
        preferred = Path(os.environ.get("LOCALAPPDATA", Path.home())) / "Silas"
    else:
        preferred = Path.home() / ".silas"

    try:
        preferred.mkdir(parents=True, exist_ok=True, mode=0o700)
        return preferred
    except OSError:
        fallback = Path(tempfile.gettempdir()) / "silas-logs"
