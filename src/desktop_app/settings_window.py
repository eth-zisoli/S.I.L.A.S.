...
def _build_field_metadata() -> List[FieldMeta]:
    ...
    f("wake_word", "Wake Word",
      "Primary wake word to activate Silas",
      "wake", "str")
    ...
    f("dictation_custom_dictionary", "Custom Dictionary",
      "Correction rules for dictation. Use 'wrong -> right' format (e.g. 'Jarvice -> Silas')",
      "features", "list")
...
class SettingsWindow(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle("⚙️ Silas Settings")
    ...
    def _build_ui(self) -> None:
        ...
        subtitle = QLabel("Changes are saved to config.json. Restart Silas to apply.")
    ...
    def _build_mcp_page(self) -> QWidget:
        ...
        # Header
        desc = QLabel(
            "MCP (Model Context Protocol) servers give Silas extra tools — "
    ...
    def _refresh_mcp_list(self) -> None:
        ...
        if self._mcp_list.count() == 0:
            self._mcp_detail.setText("No MCP servers configured. Add one to extend Silas's capabilities.")
        else:
    ...
    def _on_save(self) -> None:
        ...
        if _save_json(self._config_path, config):
            ...
            QMessageBox.information(
                self, "✅ Saved",
                "Settings saved. Restart Silas for changes to take effect."
            )
            self.accept()
        else:
            QMessageBox.warning(
