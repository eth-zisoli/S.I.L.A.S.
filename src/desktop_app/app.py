"""
Silas Desktop App - System Tray Application

A cross-platform system tray app for controlling the Silas voice assistant.
...
from silas.debug import debug_log
from silas.config import default_config_path, _default_db_path, SUPPORTED_CHAT_MODELS, get_supported_model_ids
from desktop_app.diary_dialog import DiaryUpdateDialog
from desktop_app.themes import SILAS_THEME_STYLESHEET
...
def setup_crash_logging():
    ...
    if getattr(sys, 'frozen', False):
        ...
        try:
            ...
            print(f"=== Silas Desktop App Crash Log ===", flush=True)
...
def get_crash_paths() -> tuple[Path, Path, Path]:
    ...
    crash_log = log_dir / "silas_desktop_crash.log"
...
def show_crash_report_dialog(crash_content: str) -> None:
    ...
    try:
        ...
        import webbrowser
        import urllib.parse
        from silas import get_version

        class CrashReportDialog(QDialog):
            def __init__(self, crash_info: str):
                super().__init__()
                self.crash_info = crash_info
                self.setWindowTitle("🐛 Silas Crash Report")
                self.setMinimumSize(600, 450)
                self.setStyleSheet(SILAS_THEME_STYLESHEET)
                self._setup_ui()
            ...
            def _setup_ui(self):
                ...
                # Header
                header = QLabel("😵 Silas crashed in the previous session")
...
"""
                ...
                url = f"https://github.com/isair/silas/issues/new?{params}"
...
def check_model_support() -> Optional[str]:
    ...
    """
    try:
        from silas.config import load_config, DEFAULT_CHAT_MODEL
        config = load_config()
    ...
    try:
        ...
        class UnsupportedModelDialog(QDialog):
            ...
            def _setup_ui(self):
                ...
                desc = QLabel(
                    f"You're using <b>{self.model}</b> which hasn't been tested with Silas.\n\n"
...
def get_lock_file_path() -> Path:
    ...
    if sys.platform == "darwin":
        lock_dir = Path.home() / "Library" / "Application Support" / "Silas"
    elif sys.platform == "win32":
        lock_dir = Path(os.environ.get("LOCALAPPDATA", Path.home())) / "Silas"
    else:
        lock_dir = Path.home() / ".silas"

    lock_dir.mkdir(parents=True, exist_ok=True)
    return lock_dir / "silas_desktop.lock"


def get_existing_instance_pid() -> Optional[int]:
    """Read the PID of the existing Silas instance from the lock file."""
...
def kill_existing_instance(pid: int) -> bool:
    """
    Terminate an existing Silas instance by PID.

    Returns True if the process was terminated, False otherwise.
    """
    try:
        process = psutil.Process(pid)
        # Verify it's actually a Silas process (safety check)
        proc_name = process.name().lower()
        if "silas" not in proc_name and "python" not in proc_name:
            debug_log(f"PID {pid} doesn't look like Silas (name: {proc_name}), not killing", "desktop")
            return False

        debug_log(f"Terminating existing Silas instance (PID {pid})", "desktop")
        process.terminate()
...
def show_instance_conflict_dialog() -> bool:
    ...
    msg = QMessageBox()
    msg.setWindowTitle("Silas Already Running")
    msg.setText("Another instance of Silas is already running.")
    ...
    # Apply theme
    from desktop_app.themes import SILAS_THEME_STYLESHEET
    msg.setStyleSheet(SILAS_THEME_STYLESHEET)
...
class LogViewerWindow(QMainWindow):
    """Window for viewing Silas logs in real-time."""

    def __init__(self):
        super().__init__()
        self.setWindowTitle("📝 Silas Logs")
        ...
        # Initial message
        self.append_log("🚀 Silas Log Viewer Ready\n" + _LOG_SEPARATOR + "\n\n")
    ...
    def _report_issue(self) -> None:
        ...
        from silas import get_version
        from silas.utils.redact import _REDACTION_RULES
...
class MemoryViewerWindow(QMainWindow):
    """Window for viewing Silas memory using embedded web view."""

    MEMORY_VIEWER_PORT = 5050

    def __init__(self):
        super().__init__()
        self.setWindowTitle("🧠 Silas Memory")
...
class SilasSystemTray:
    """System tray application for Silas voice assistant."""
    ...
    def __init__(self):
        ...
        # Kill any orphaned Silas processes from previous sessions
        ...
        from silas.dictation.history import DictationHistory
    ...
    def cleanup_orphaned_processes(self) -> None:
        """Kill any orphaned Silas daemon processes from previous sessions."""
        try:
            current_pid = os.getpid()
            for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                try:
                    cmdline = proc.info.get('cmdline', [])
                    if cmdline and 'silas.main' in ' '.join(cmdline):
                        # This is a Silas daemon process
                        if proc.pid != current_pid:
                            debug_log(f"killing orphaned silas process: {proc.pid}", "desktop")
                            proc.terminate()
                            try:
                                proc.wait(timeout=2)
                            except psutil.TimeoutExpired:
    ...
    def _run_cuda_recovery(self, action_spec: "CudaRecoveryAction") -> None:
        ...
        reply = QMessageBox.question(
            ...
            (
                ...
                "into the Silas program folder. You'll see a UAC prompt. "
                "Continue?"
            ),
        ...
        QMessageBox.information(
            ...
            (
                ...
                f"Silas to use GPU acceleration. See {action_spec.target_dir / 'install.log'} "
                "for details."
            ),
        )
    ...
    def show_setup_wizard(self) -> None:
        ...
        try:
            from desktop_app.face_widget import SilasState, get_silas_state
            get_silas_state().set_state(SilasState.ASLEEP)
        except Exception:
            pass
    ...
    def show_settings(self) -> None:
        ...
        if result == QDialog.DialogCode.Accepted and self.is_listening:
            reply = QMessageBox.question(
                None, "🔄 Restart?",
                "Settings saved. Restart Silas now to apply changes?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
    ...
    def _connect_dictation_history(self, retries_left: int = 3) -> None:
        ...
        """
        try:
            from silas.daemon import get_dictation_engine
    ...
    def toggle_listening(self) -> None:
        """Toggle the Silas daemon on/off."""
        if self.is_listening:
            self.stop_daemon()
        else:
            self.start_daemon()
                    ...
                    def run(self):
                        ...
                        try:
                            ...
                            try:
                                ...
                                from silas.daemon import main as daemon_main
                                self.log_signals.new_log.emit("🚀 Silas daemon started\n")
                                self.log_signals.new_log.emit("📋 Initializing daemon components...\n")

                                # Run daemon - this should run the main loop
                                daemon_main()

                                from silas.daemon import is_stop_requested
    ...
    def stop_daemon(self, show_diary_dialog: bool = True) -> None:
        ...
        try:
            if self.is_bundled and self.daemon_thread:
                ...
                self.log_signals.new_log.emit("⏸️ Stopping Silas daemon...\n")
                ...
                if show_diary_dialog:
                    ...
                    from silas.daemon import set_diary_update_callbacks, request_stop
                    ...
                    # Note: We avoid QThread.terminate() as it can corrupt state
                    from silas.daemon import request_stop
                    request_stop()
            ...
            self.log_signals.new_log.emit("⏸️ Silas daemon stopped\n")
...
def main() -> int:
    ...
    if not acquire_single_instance_lock():
        print("⚠️ Another instance of Silas Desktop is already running.", flush=True)
    ...
    print("Starting Silas Desktop App...", flush=True)
    ...
    try:
        ...
        splash.set_status("Loading Silas...")
        print("Initializing SilasSystemTray...", flush=True)
        tray_instance = SilasSystemTray()
        print("SilasSystemTray initialized successfully", flush=True)

        # Always auto-start listening (logs will be shown via start_daemon)
        splash.set_status("Starting voice assistant...")
        print("🚀 Auto-starting Silas listener...", flush=True)
        ...
        try:
            ...
            msg.setWindowTitle("Silas Desktop App Error")
            msg.setText("Failed to start Silas Desktop App")
