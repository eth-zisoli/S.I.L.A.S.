"""
Low-poly grid face widget for Silas with intelligent state management and organic idle behavior.
...
class SilasState(Enum):
    """Overall Silas state for face animation."""
...
# Global Silas state - allows daemon to signal overall state to face widget
# Uses a file-based approach to work across processes (dev mode runs daemon as subprocess)
import tempfile
import os

def _get_silas_state_file() -> str:
    """Get the path to the Silas state file."""
    return os.path.join(tempfile.gettempdir(), "silas_state")


class SilasStateManager(QObject):
    """Global singleton for Silas state management.
    ...
    def __init__(self):
        super().__init__()
        self._state = SilasState.ASLEEP  # Start asleep
        self._state_lock = threading.Lock()
        self._state_file = _get_silas_state_file()
        ...
        self._write_state(SilasState.ASLEEP)

    @property
    def state(self) -> SilasState:
    ...
    def _write_state(self, state: SilasState) -> None:
    ...
    def set_state(self, state: SilasState) -> None:
        """Set the Silas state (thread-safe, cross-process)."""
        with self._state_lock:
            self._state = state
...
_silas_state_instance: Optional[SilasStateManager] = None
_silas_state_lock = threading.Lock()


def get_silas_state() -> SilasStateManager:
    """Get the global Silas state singleton."""
    global _silas_state_instance
    with _silas_state_lock:
        if _silas_state_instance is None:
            _silas_state_instance = SilasStateManager()
        return _silas_state_instance
...
class LowPolyFaceWidget(QWidget):
    ...
    # Colors
    PRIMARY_COLOR = QColor("#fbbf24")  # Amber/gold - matches Silas theme
    ...
    def __init__(self, parent=None):
        ...
        # Current Silas state
        self._silas_state = SilasState.ASLEEP  # Start asleep until daemon ready
        ...
        # Connect to global Silas state
        self._state_manager = get_silas_state()
    ...
    def _on_state_changed(self, state_value: str):
        """Handle Silas state change from global state."""
        try:
            self._silas_state = SilasState(state_value)
        except ValueError:
            pass
    ...
    def _animate(self):
        ...
        # Poll Silas state directly (more reliable than cross-thread signals)
        prev_state = self._silas_state
        try:
            self._silas_state = self._state_manager.state
        except Exception:
            pass
        ...
        if (self._silas_state == SilasState.LISTENING
                and prev_state != SilasState.LISTENING):
            self._listening_started_at = _time.monotonic()
            self._listening_rings_spawned = 0

        # Update activation level based on state
        if self._silas_state == SilasState.ASLEEP:
            self._target_activation = 0.0
        else:
        ...
        is_idle = self._silas_state == SilasState.IDLE and self._activation_level > 0.5
        ...
        if self._silas_state == SilasState.SPEAKING:
            self._gaze_x *= 0.95
            self._gaze_y *= 0.95
        ...
        pulse_cycle_s = 2.0
        ring_lifespan_s = 2.0
        if self._silas_state == SilasState.LISTENING and self._listening_started_at is not None:
        ...
        if self._silas_state in (SilasState.DICTATING, SilasState.DICTATION_PROCESSING):
        ...
        if self._silas_state in (SilasState.THINKING, SilasState.DICTATION_PROCESSING):
    ...
    def _draw_dictation_pulse(self, painter: QPainter, cx: float, cy: float,
        ...
        if self._silas_state not in (SilasState.DICTATING, SilasState.DICTATION_PROCESSING):
            return
...
class FaceWindow(QWidget):
    """A standalone window containing the Silas face."""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.setWindowTitle("🤖 Silas")
