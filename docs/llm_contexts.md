...
Every distinct LLM call in Silas, what feeds it, what consumes it, and how it is gated. This is the reference for optimising the app's main bottleneck (LLM latency). Keep it in sync with the code — see the note at the bottom.

---

## 1. Main Reply Loop (agentic messages loop)

- **File**: [src/silas/reply/engine.py](src/silas/reply/engine.py) — `reply()` and the loop at ~lines 1370-1650; native tool-call path in `chat_with_messages()` (~1424, 1455).
...
- **Inputs**:
  - Redacted user query
  - Recent dialogue (last 5 minutes), including in-loop tool-call + tool-role messages from prior replies within the active conversation (tool carryover, `DialogueMemory.record_tool_turn` / `get_recent_turns_with_tools` in [src/silas/memory/conversation.py](src/silas/memory/conversation.py); per-prompt cap via `cfg.tool_carryover_max_turns` / `tool_carryover_per_entry_chars`; storage cap `_tool_turns_max_storage = 16`; cleared on `stop` signal AND on new-conversation entry; UNTRUSTED WEB EXTRACT fence markers preserved on truncation; both `content` and `tool_calls[*].function.arguments` scrubbed on write)
  - Unified system prompt from [src/silas/system_prompt.py](src/silas/system_prompt.py) + ASR note + tool-protocol guidance
  - **Warm profile block** (query-agnostic User + Directives excerpt from the knowledge graph, composed by `build_warm_profile()` / `format_warm_profile_block()` in [src/silas/memory/graph_ops.py](src/silas/memory/graph_ops.py) at Step 3.5 of `reply()`; no LLM call, pure SQLite read; injected unconditionally so personalisation is the default; result cached in `DialogueMemory._hot_cache` under `DialogueMemory.WARM_PROFILE_CACHE_KEY` for the lifetime of the active conversation. Invalidated on `stop`, on new-conversation entry, AND on User/Directives graph mutations via the listener registered in [src/silas/daemon.py](src/silas/daemon.py) against `register_graph_mutation_listener` in [src/silas/memory/graph.py](src/silas/memory/graph.py); World-branch writes are ignored)
  - Digested memory enrichment (optional, see #4)
  - Time + location context (re-injected each turn)
  - Tool schema: native via `generate_tools_json_schema()` ([src/silas/tools/registry.py](src/silas/tools/registry.py)) or text fallback via `_text_tool_call_guidance()` ([engine.py:68](src/silas/reply/engine.py:68))
...
- **File**: [src/silas/listening/intent_judge.py](src/silas/listening/intent_judge.py) — `IntentJudge.evaluate()`.
...
- **System prompt**: `SYSTEM_PROMPT_TEMPLATE` at [intent_judge.py:135](src/silas/listening/intent_judge.py:135). Teaches query extraction, echo detection, stop commands, pronoun/topic disambiguation, imperative re-addressing, declaratives to the wake word.
- **Output**: strict JSON `IntentJudgment{directed, query, stop, confidence, reasoning}` ([intent_judge.py:94](src/silas/listening/intent_judge.py:94)). Consumed by the listening state machine which dispatches to the reply engine.
...
- **File**: [src/silas/reply/enrichment.py](src/silas/reply/enrichment.py) — `extract_search_params_for_memory()` (~line 71).
...
- **System prompt**: inline at [enrichment.py:35-63](src/silas/reply/enrichment.py:35).
...
- **File**: [src/silas/memory/recall_gate.py](src/silas/memory/recall_gate.py) — `should_recall()`.
...
- **File**: [src/silas/reply/enrichment.py](src/silas/reply/enrichment.py) — `digest_memory_for_query()` + `_distil_batch()`.
...
- **System prompt**: `_DIGEST_SYSTEM_PROMPT` at [enrichment.py:122](src/silas/reply/enrichment.py:122). Teaches relevance filtering, preference-signal detection, attribution preservation, `NONE` sentinel, identity queries.
...
- **File**: [src/silas/reply/enrichment.py](src/silas/reply/enrichment.py) — `digest_tool_result_for_query()` + `_distil_tool_batch()`.
...
- **File**: [src/silas/reply/enrichment.py](src/silas/reply/enrichment.py) — `digest_loop_for_max_turns()` (~line 847).
...
- **File**: [src/silas/tools/selection.py](src/silas/tools/selection.py) — `select_tools_with_llm()` (~line 331).
...
- **File**: [src/silas/memory/conversation.py](src/silas/memory/conversation.py) — `generate_conversation_summary()` (~lines 350/355).
...
- **File**: [src/silas/memory/graph_ops.py](src/silas/memory/graph_ops.py) — `extract_graph_memories()`.
...
- **File**: [src/silas/memory/graph_ops.py](src/silas/memory/graph_ops.py) — `_llm_pick_best_child()` (~line 167).
...
- **File**: [src/silas/memory/graph_ops.py](src/silas/memory/graph_ops.py) — `merge_node_data()` (system prompt at `_MERGE_SYSTEM_PROMPT`).
...
- **File**: [src/silas/reply/planner.py](src/silas/reply/planner.py) — `plan_query()`.
...
- **File**: [src/silas/reply/planner.py](src/silas/reply/planner.py) — `resolve_next_tool_call()`.
...
- **System prompt**: `_STEP_RESOLVER_SYSTEM` at [planner.py:300](src/silas/reply/planner.py:300). Teaches one-JSON-object output, placeholder substitution from prior results, `null` for synthesis steps.
...
- **Weather** ([src/silas/tools/builtin/weather.py](src/silas/tools/builtin/weather.py), ~line 60) — `ollama_chat_model`, parses location/time/unit from the query.
- **Nutrition log_meal** ([src/silas/tools/builtin/nutrition/log_meal.py](src/silas/tools/builtin/nutrition/log_meal.py), lines 48 & 136) — `ollama_chat_model`, extracts nutrients, confirms logging.
