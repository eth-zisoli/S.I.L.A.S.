...
from silas.listening.wake_detection import (
...
class TestWakeWordDetection:
    ...
    def test_exact_match(self):
        assert is_wake_word_detected("hey silas", "silas", []) is True

    def test_alias_match(self):
        assert is_wake_word_detected("hey computer", "silas", ["computer"]) is True

    def test_no_match(self):
        assert is_wake_word_detected("hello world", "silas", []) is False

    def test_empty_text(self):
        assert is_wake_word_detected("", "silas", []) is False

    def test_fuzzy_match(self):
        """Fuzzy matching catches slight transcription errors."""
        assert is_wake_word_detected("hey jarvas", "silas", [], fuzzy_ratio=0.78) is True

    def test_fuzzy_below_threshold(self):
        """Completely different word doesn't fuzzy-match."""
        assert is_wake_word_detected("hey banana", "silas", [], fuzzy_ratio=0.78) is False
...
class TestExtractQueryAfterWake:
    ...
    def test_extracts_query(self):
        result = extract_query_after_wake("silas what time is it", "silas", [])
        assert result == "what time is it"

    def test_extracts_query_with_alias(self):
        result = extract_query_after_wake("hey computer what time is it", "silas", ["hey computer"])
        assert result == "what time is it"

    def test_wake_word_only_returns_empty(self):
        """When only the wake word is said, return empty string (no hardcoded fallback)."""
        result = extract_query_after_wake("silas", "silas", [])
        assert result == ""
    ...
    def test_empty_text(self):
        result = extract_query_after_wake("", "silas", [])
        assert result == ""

    def test_strips_leading_punctuation(self):
        result = extract_query_after_wake("silas, tell me a joke", "silas", [])
