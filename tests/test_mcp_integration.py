...
def test_mcp_tools_integrated_with_reply_engine():
    ...
    from silas.tools.registry import discover_mcp_tools, generate_tools_description, BUILTIN_TOOLS
    ...
    with patch('silas.tools.registry.MCPClient', FakeMCPClient):
        # Test discovery
...
def test_mcp_tool_execution_in_context():
    ...
    from silas.tools.registry import run_tool_with_retries, ToolExecutionResult
...
def test_mcp_error_handling_in_context():
    ...
    from silas.tools.registry import run_tool_with_retries
    ...
    with patch('silas.tools.registry.MCPClient', FailingMCPClient):
        result = run_tool_with_retries(
            db=MockDB(),
            cfg=MockConfig(),
