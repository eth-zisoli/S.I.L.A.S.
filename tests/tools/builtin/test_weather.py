...
from src.silas.tools.builtin.weather import (
    WeatherTool,
    WMO_CODES,
...
class TestWeatherTool:
    ...
    @patch('src.silas.tools.builtin.weather.get_location_info')
    ...
    @patch('requests.get')
    @patch('src.silas.tools.builtin.weather._extract_place_from_user_text')
    @patch('src.silas.tools.builtin.weather.get_location_info')
...
class TestExtractPlaceFromUserText:
    ...
    @patch("src.silas.tools.builtin.weather.call_llm_direct", create=True)
    def test_extracts_clean_place_name(self, _mock_direct):
        """Patch the import inside the function by intercepting call_llm_direct."""
        from src.silas.llm import call_llm_direct as real_fn  # noqa: F401

        with patch("src.silas.llm.call_llm_direct", return_value="London"):
            got = _extract_place_from_user_text("I need it for London", self._cfg())
        assert got == "London"

    def test_strips_quotes_and_punctuation(self):
        with patch("src.silas.llm.call_llm_direct", return_value="'Paris'."):
    ...
    def test_none_sentinel_returns_none(self):
        for sentinel in ("none", "None", "NONE", "n/a", "unknown"):
            with patch("src.silas.llm.call_llm_direct", return_value=sentinel):
    ...
    def test_overlong_response_rejected(self):
        with patch("src.silas.llm.call_llm_direct", return_value="x" * 200):
