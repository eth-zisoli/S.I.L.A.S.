...
from silas.listening.wake_detection import is_wake_word_detected
...
class TestWakeWordValidation:
    ...
    def test_wake_word_detected_with_silas(self):
        """Wake word detection finds 'silas' in text."""
        text = "hey silas what time is it"
        assert is_wake_word_detected(text, "silas", []) is True

    def test_wake_word_detected_with_alias(self):
        """Wake word detection finds alias."""
        text = "hey assistant what time is it"
        assert is_wake_word_detected(text, "silas", ["assistant"]) is True

    def test_wake_word_not_detected_without_wake_word(self):
        """Wake word detection returns False when no wake word present."""
        text = "how are you"
        assert is_wake_word_detected(text, "silas", []) is False
    ...
    def test_wake_word_not_detected_similar_but_different(self):
        ...
        # "jarring" is similar to "silas" but should not match with high threshold
        assert is_wake_word_detected(text, "silas", [], fuzzy_ratio=0.9) is False
    ...
    def test_bug_scenario_no_wake_word_in_query(self):
        ...
        if not could_be_hot_window:
            ...
            # This should be False - there's no "silas" in "how are you"
...
class TestIntentJudgeWakeWordValidation:
    ...
    def test_intent_judge_directed_accepted_with_wake_word(self):
        ...
        text_lower = "silas what's the weather"
        could_be_hot_window = False  # Wake word mode
...
class TestWakeTimestampCapture:
    ...
    def test_wake_timestamp_set_on_wake_word_detection(self):
        ...
        text_lower = "silas what's the weather tomorrow"
...
class TestHotWindowQueryValidation:
    ...
    def test_interrupt_during_tts(self):
        ...
        current_segment = "silas stop talking"
        wake_word = "silas"
...
class TestQueryValidationNotUsed:
    ...
    def test_false_negative_synthesized_query_context_heavy(self):
        ...
        Multi-person conversation about iPhone, user asks "Silas how much"
        ...
        text_without_wake = text.replace("silas", "").strip()  # "how much"
        ...
        text2 = "silas what's the price"
        text2_without_wake = text2.replace("silas", "").strip()  # "what's the price"
    ...
    def test_wake_word_check_is_reliable(self):
        ...
        - Wake word check correctly rejects (no "silas")

        If user says "silas what do you think":
        - Wake word check correctly accepts (has "silas")
        - LLM can synthesize any query it wants
        """
        # Case 1: No wake word - correctly rejected
        text_no_wake = "how are you"
        assert is_wake_word_detected(text_no_wake, "silas", []) is False

        # Case 2: Has wake word - correctly accepted
        text_with_wake = "silas what do you think"
        assert is_wake_word_detected(text_with_wake, "silas", []) is True
