...
def shutdown_persistent_runtime():
    ...
    yield
    try:
        from silas.tools.external.mcp_runtime import shutdown_runtime
        shutdown_runtime()
    except Exception:
        pass
...
def _patch_mcp_doubles(monkeypatch, TrackedConn, TrackedSession):
    monkeypatch.setattr(
        "silas.tools.external.mcp_client._resolve_command", lambda c: c
    )
    monkeypatch.setattr(
        "silas.tools.external.mcp_client.stdio_client",
        lambda params, **kw: TrackedConn(),
    )
    monkeypatch.setattr(
        "silas.tools.external.mcp_client.ClientSession", TrackedSession
    )
...
def test_invoke_tool_keeps_mcp_session_alive_across_calls(monkeypatch, shutdown_persistent_runtime):
    ...
    """
    from silas.tools.external.mcp_client import MCPClient
...
def test_invoke_tool_retries_on_transient_session_loss(
    ...
    """
    from silas.tools.external.mcp_client import MCPClient
    from silas.tools.external import mcp_runtime as _runtime_mod
...
def test_worker_startup_failure_propagates(monkeypatch, shutdown_persistent_runtime):
    ...
    """
    from silas.tools.external.mcp_client import (
        MCPClient,
    ...
    monkeypatch.setattr(
        "silas.tools.external.mcp_client.stdio_client", _broken_stdio_client
    )
...
def test_absolute_path_command_skips_which(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("silas.tools.external.mcp_client.shutil.which", tracking_which)
    ...
    monkeypatch.setattr("silas.tools.external.mcp_client.stdio_client", lambda params, **kw: FakeCM())
    monkeypatch.setattr("silas.tools.external.mcp_client.ClientSession", FakeSession)
...
def test_mcp_client_list_and_invoke(monkeypatch):
    ...
    monkeypatch.setattr("silas.tools.external.mcp_client.stdio_client", lambda params, **kw: FakeCM(FakeSession()))
    monkeypatch.setattr("silas.tools.external.mcp_client.ClientSession", FakeClientSession)
    # Avoid PATH check failing in _connect_stdio
    monkeypatch.setattr("silas.tools.external.mcp_client.shutil.which", lambda cmd: cmd)
...
class TestResolveCommand:
    ...
    def test_finds_command_on_path(self, monkeypatch):
        ...
        monkeypatch.setattr("silas.tools.external.mcp_client.shutil.which", lambda cmd: "/usr/bin/npx")
    ...
    def test_finds_command_in_extra_dirs(self, monkeypatch, tmp_path):
        ...
        from silas.tools.external.mcp_client import _resolve_command
        monkeypatch.setattr("silas.tools.external.mcp_client.shutil.which", lambda cmd: None)
        ...
        monkeypatch.setattr(
            "silas.tools.external.mcp_client._EXTRA_PATH_DIRS",
            [str(tmp_path)],
        )
        monkeypatch.setattr("silas.tools.external.mcp_client._EXTRA_PATH_GLOBS", [])
        # Skip login shell fallback
        monkeypatch.setattr("silas.tools.external.mcp_client._sys.platform", "win32")
    ...
    def test_falls_back_to_login_shell(self, monkeypatch):
        ...
        monkeypatch.setattr("silas.tools.external.mcp_client.shutil.which", lambda cmd: None)
        monkeypatch.setattr("silas.tools.external.mcp_client._EXTRA_PATH_DIRS", [])
        monkeypatch.setattr("silas.tools.external.mcp_client._EXTRA_PATH_GLOBS", [])
        monkeypatch.setattr("silas.tools.external.mcp_client._sys.platform", "darwin")
    ...
    def test_finds_command_via_nvm_glob(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "silas.tools.external.mcp_client._EXTRA_PATH_GLOBS",
            [str(tmp_path / "*/bin")],
        )
...
class TestConnectStdioPathInjection:
    ...
    def test_command_dir_added_to_env_path(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "silas.tools.external.mcp_client._resolve_command",
