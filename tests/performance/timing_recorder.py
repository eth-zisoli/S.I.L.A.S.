...
Monkey-patches the three entry points in ``silas.llm`` (``call_llm_direct``,
...
from silas import llm as _llm_module
...
class TimingRecorder:
    ...
    def _patch(self) -> None:
        ...
        import time and do NOT see a setattr on ``silas.llm`` itself, so we
        ...
        for mod in list(_sys.modules.values()):
            ...
            if not mod_name.startswith(("silas", "tests", "evals")):
                continue
            for name in names:
        ...
        # Also patch the canonical module so any late `from silas.llm import X`
