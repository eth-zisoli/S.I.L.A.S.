...
|-----|---------|-------------|
| `SILAS_PERF_OLLAMA_URL` | `http://localhost:11434` | Ollama endpoint |
| `SILAS_PERF_MODEL` | `gemma4:e2b` | Model pulled in Ollama for the run |
| `SILAS_PERF_RUNS` | `3` | Runs per query (bump for tighter p95) |
| `SILAS_PERF_REPORT_DIR` | `tests/performance/reports/` | JSON report output |

`PERF_RUNS=3` is a fast-iteration default. For stable p95 numbers when
benchmarking a change, use `SILAS_PERF_RUNS=10` or higher.
