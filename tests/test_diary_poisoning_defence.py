...
class TestSummariserForbidsDeflectionNarration:
    ...
    def _capture_system_prompt(self) -> str:
        ...
        with patch('silas.llm.call_llm_direct', side_effect=fake_call):
    ...
    def test_prompt_forbids_welding_unrelated_topics(self):
        ...
        Field DB contained a diary entry reading:
          "The conversation focused on the movie 'Possessor' and the character
           'Silas,' identified as the artificial intelligence from the
        ...
        AI character named Silas) were welded into one clause via "and" plus
        ...
        # Concrete good/bad example using the field-observed Possessor/Silas
        # case (the same one used elsewhere in the prompt — but here about
        # topic separation, not attribution).
        assert "silas" in lowered and "possessor" in lowered, (
            "Summariser prompt should include the Possessor/Silas topic-welding "
            "BAD→GOOD example."
        )
...
class TestRewriteDeflectionSystemPrompt:
    ...
    def _prompt(self) -> str:
        from silas.memory.conversation import _REWRITE_DEFLECTION_SYSTEM_PROMPT
...
class TestDiaryEnrichmentInjectionFraming:
    ...
    def test_engine_injects_diary_under_reference_only_label(self):
        ...
        import inspect
        from silas.reply import engine
