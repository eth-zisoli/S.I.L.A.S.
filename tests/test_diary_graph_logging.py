...
from silas.memory.graph_ops import GraphUpdateResult
...
class TestKnowledgeGraphConsoleLogging:
    ...
    def _run_flush(self, db, dialogue_memory, graph_result):
        ...
        """
        from silas.memory.conversation import update_diary_from_dialogue_memory
        ...
        with patch(
            "silas.memory.conversation.generate_conversation_summary",
            return_value=("User asked about bats. Bats are not blind.", "bats, biology"),
        ), patch(
            "silas.memory.graph_ops.update_graph_from_dialogue",
