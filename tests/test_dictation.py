...
def _make_engine(**overrides):
    ...
    from src.silas.dictation.dictation_engine import DictationEngine
...
class TestBeepGeneration:
    ...
    def test_start_beep_is_valid_wav(self):
        from src.silas.dictation.dictation_engine import _get_start_beep
        wav = _get_start_beep()
        assert wav[:4] == b"RIFF"
        assert wav[8:12] == b"WAVE"

    def test_stop_beep_is_valid_wav(self):
        from src.silas.dictation.dictation_engine import _get_stop_beep
        wav = _get_stop_beep()
        assert wav[:4] == b"RIFF"
        assert wav[8:12] == b"WAVE"

    def test_start_and_stop_beeps_differ(self):
        from src.silas.dictation.dictation_engine import _get_start_beep, _get_stop_beep
        assert _get_start_beep() != _get_stop_beep()

    def test_generate_beep_wav_custom_params(self):
        from src.silas.dictation.dictation_engine import _generate_beep_wav
...
class TestHotkeyParsing:
    ...
    def test_parse_ctrl_shift_d(self):
        from src.silas.dictation.dictation_engine import parse_hotkey
...
class TestEngineLifecycle:
    ...
    @patch("src.silas.dictation.dictation_engine.platform")
    @patch("src.silas.dictation.dictation_engine.sys")
    @patch("src.silas.dictation.dictation_engine.pynput_keyboard")
    ...
    @patch("src.silas.dictation.dictation_engine.pynput_keyboard", None)
    ...
    @patch("src.silas.dictation.dictation_engine.sd", None)
    @patch("src.silas.dictation.dictation_engine.pynput_keyboard")
...
class TestRecordingStateMachine:
    ...
    def test_start_recording_allows_mlx_without_model(self):
        ...
        with patch("src.silas.dictation.dictation_engine.sd") as mock_sd, \
             patch("src.silas.dictation.dictation_engine._play_beep"):
    ...
    def test_stop_recording_returns_fast_on_slow_stream_close(self):
        ...
        with patch("src.silas.dictation.dictation_engine._play_beep"):
            t0 = time.time()
    ...
    def test_finalise_fires_on_dictation_end_when_beep_raises(self):
        ...
        with patch(
            "src.silas.dictation.dictation_engine._play_beep",
...
class TestClipboard:
    ...
    @patch("src.silas.dictation.dictation_engine.platform")
    @patch("src.silas.dictation.dictation_engine._clipboard_windows")
    @patch("src.silas.dictation.dictation_engine.pynput_keyboard")
    def test_clipboard_paste_windows(self, mock_kb, mock_clip_win, mock_platform):
        from src.silas.dictation.dictation_engine import _clipboard_paste
    ...
    @patch("src.silas.dictation.dictation_engine._paste_cgevent", return_value=True)
    @patch("src.silas.dictation.dictation_engine._check_macos_accessibility", return_value=True)
    @patch("src.silas.dictation.dictation_engine.platform")
    @patch("src.silas.dictation.dictation_engine._clipboard_macos")
    @patch("src.silas.dictation.dictation_engine.pynput_keyboard")
...
class TestTranscribeAndPaste:
    ...
    @patch("src.silas.dictation.dictation_engine._clipboard_paste")
...
class TestConfigIntegration:
    ...
    def test_settings_has_dictation_fields(self):
        from src.silas.config import Settings
        import inspect
    ...
    def test_default_config_has_dictation(self):
        import sys
        from src.silas.config import get_default_config
    ...
    def test_load_settings_includes_dictation(self):
        ...
        from src.silas.config import load_settings
...
class TestFaceWidgetDictatingState:
    ...
    def test_silas_state_has_dictating(self):
        from src.desktop_app.face_widget import SilasState
        assert hasattr(SilasState, "DICTATING")
        assert SilasState.DICTATING.value == "dictating"

    def test_dictating_state_round_trips(self):
        """State manager should accept DICTATING state."""
        from src.desktop_app.face_widget import SilasState
        state = SilasState("dictating")
        assert state == SilasState.DICTATING

    def test_silas_state_has_dictation_processing(self):
        from src.desktop_app.face_widget import SilasState
        assert hasattr(SilasState, "DICTATION_PROCESSING")
        assert SilasState.DICTATION_PROCESSING.value == "dictation_processing"

    def test_dictation_processing_state_round_trips(self):
        from src.desktop_app.face_widget import SilasState
        state = SilasState("dictation_processing")
        assert state == SilasState.DICTATION_PROCESSING
...
class TestDictationProcessingCallback:
    ...
    def test_processing_callback_fires_before_end_callback(self):
        ...
        from src.silas.dictation import dictation_engine as de
...
class TestListenerPauseFlag:
    ...
    def listener(self):
        ...
        from src.silas.listening.listener import VoiceListener
        cfg = MagicMock()
...
class TestFormatHotkeyDisplay:
    ...
    def test_windows_cmd_shows_win(self, mock_platform):
        from src.silas.dictation.dictation_engine import format_hotkey_display
        mock_platform.system.return_value = "Windows"
...
class TestClipboardWindowsCtypes:
    ...
    def test_clipboard_windows_roundtrip(self):
        ...
        from src.silas.dictation.dictation_engine import _clipboard_windows
