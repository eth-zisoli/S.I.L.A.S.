...
class TestFaceWidgetImports:
    ...
    def test_face_widget_importable_from_desktop_app(self):
        ...
        from desktop_app.face_widget import get_silas_state, SilasState
        assert get_silas_state is not None
        assert SilasState is not None

    @pytest.mark.unit
    def test_silas_state_enum_has_expected_values(self):
        """SilasState enum should have all expected states."""
        from desktop_app.face_widget import SilasState

        expected_states = ['ASLEEP', 'IDLE', 'LISTENING', 'THINKING', 'SPEAKING']
        for state in expected_states:
            assert hasattr(SilasState, state), f"SilasState missing {state}"

    @pytest.mark.unit
    def test_tts_module_face_widget_import(self):
        """TTS module's face_widget import should work.

        This tests the actual import path used in silas/output/tts.py
        """
    ...
    def test_listener_module_face_widget_import(self):
        ...
        This tests the actual import path used in silas/listening/listener.py
        """
        try:
            from desktop_app.face_widget import get_silas_state, SilasState
            success = True
    ...
    def test_state_manager_module_face_widget_import(self):
        ...
        This tests the actual import path used in silas/listening/state_manager.py
        """
        try:
            from desktop_app.face_widget import get_silas_state, SilasState
            success = True
    ...
    def test_reply_engine_module_face_widget_import(self):
        ...
        This tests the actual import path used in silas/reply/engine.py
        """
        try:
            from desktop_app.face_widget import get_silas_state, SilasState
            success = True
...
class _HeadlessStateManager:
    """Lightweight stand-in for SilasStateManager that works without Qt.
    ...
    def __init__(self):
        import threading
        from desktop_app.face_widget import SilasState, _get_silas_state_file
        self._state = SilasState.ASLEEP
        ...
        try:
            if os.path.exists(self._state_file):
                with open(self._state_file, 'r') as f:
                    return SilasState(f.read().strip())
...
class TestSilasStateManager:
    """Tests for SilasStateManager cross-process state sharing."""

    @pytest.fixture(autouse=True)
    def cleanup_state_file(self):
        """Clean up state file and singleton before/after each test.

        Replaces get_silas_state with a headless factory so tests work
        ...
        state_file = os.path.join(tempfile.gettempdir(), "silas_state")

        # Reset singleton before test
        face_widget._silas_state_instance = None
        ...
        _orig_get = face_widget.get_silas_state

        def _headless_get():
            if face_widget._silas_state_instance is None:
                face_widget._silas_state_instance = _HeadlessStateManager()
            return face_widget._silas_state_instance

        face_widget.get_silas_state = _headless_get

        yield

        face_widget.get_silas_state = _orig_get
        face_widget._silas_state_instance = None
    ...
    def test_state_manager_creates_file_if_not_exists(self):
        ...
        # face_widget.get_silas_state() to pick up the fixture's patch.
        sm = face_widget.get_silas_state()

        # File should now exist
        assert os.path.exists(state_file)

        # Default state should be ASLEEP
        assert sm.state == SilasState.ASLEEP
    ...
    def test_state_manager_file_based_sharing(self):
        ...
        sm = face_widget.get_silas_state()
        sm.set_state(SilasState.SPEAKING)
        ...
        assert sm.state == SilasState.SPEAKING
        ...
        assert sm.state == SilasState.THINKING
