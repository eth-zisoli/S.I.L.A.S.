    ...
    def test_macos_arm64(self):
        with patch("sys.platform", "darwin"):
            with patch("platform.machine", return_value="arm64"):
                assert get_platform_asset_name() == "Silas-macOS-arm64.zip"

    @pytest.mark.unit
    def test_macos_x64(self):
        with patch("sys.platform", "darwin"):
            with patch("platform.machine", return_value="x86_64"):
                assert get_platform_asset_name() == "Silas-macOS-x64.zip"

    @pytest.mark.unit
    def test_windows(self):
        with patch("sys.platform", "win32"):
            assert get_platform_asset_name() == "Silas-Windows-x64.zip"

    @pytest.mark.unit
    def test_linux(self):
        with patch("sys.platform", "linux"):
            assert get_platform_asset_name() == "Silas-Linux-x64.tar.gz"
        ...
        mock_response.json.return_value = [
            {
                ...
                "draft": False,
                "prerelease": False,
                "html_url": "https://github.com/isair/silas/releases/tag/v1.0.0",
                "body": "Release notes",
                "assets": [
                    {
                        "id": 100001,
                        "name": "Silas-macOS-arm64.zip",
        ...
        mock_response.json.return_value = [
            {
                ...
                "draft": False,
                "prerelease": False,
                "html_url": "https://github.com/isair/silas/releases/tag/v1.1.0",
                "body": "Release notes",
                "assets": [
                    {
                        "id": 100002,
        ...
        mock_response.json.return_value = [
            {
                ...
                "draft": False,
                "prerelease": True,
                "html_url": "https://github.com/isair/silas/releases/tag/latest",
        ...
        mock_response.json.return_value = [
            {
                ...
                "prerelease": False,
                "html_url": "https://github.com/isair/silas/releases/tag/v2.0.0",
                "body": "Release notes",
                "assets": [
                    {
                        "id": 100004,
        ...
        mock_response.json.return_value = [
            {
                ...
                "assets": [
                    {
                        "id": 100005,
                        "name": "Silas-Windows-x64.zip",  # Only Windows asset
                        "browser_download_url": "https://example.com/download",
                        "size": 1000,
                    }
                ],
...
class TestUpdateStatus:
    ...
    def test_update_status_fields(self):
        release = ReleaseInfo(
            ...
            asset_name="Silas-macOS-arm64.zip",
            asset_size=1000000,
        ...
        mock_response.json.return_value = [
            {
                ...
                "assets": [{"id": 300, "name": "Silas-macOS-arm64.zip",
            ...
            },
            {
                ...
                "body": "* fix: bug two (#2)",
                "assets": [{"id": 200, "name": "Silas-macOS-arm64.zip",
            ...
            },
            {
                ...
                "assets": [{"id": 100, "name": "Silas-macOS-arm64.zip",
...
class TestReleaseInfo:
    ...
    def test_release_info_fields(self):
        release = ReleaseInfo(
            ...
            name="Version 1.2.3",
            prerelease=False,
            html_url="https://github.com/isair/silas/releases/tag/v1.2.3",
            download_url="https://github.com/isair/silas/releases/download/v1.2.3/Silas.zip",
            asset_name="Silas-macOS-arm64.zip",
            asset_size=52428800,
...
class TestInstallUpdateWindows:
    ...
    def test_batch_script_waits_for_pid(self, tmp_path):
        ...
        # Create a mock zip file with Silas.exe
        zip_path = tmp_path / "update.zip"
        with zipfile.ZipFile(zip_path, "w") as zf:
            zf.writestr("Silas.exe", b"mock executable content")

        # Mock get_app_path to return a fake path
        mock_app_path = tmp_path / "Silas.exe"
    ...
    def test_batch_script_launches_updated_exe(self, tmp_path):
        ...
        mock_app_path = tmp_path / "Program Files" / "Silas" / "Silas.exe"
...
class TestInstallUpdateMacos:
    ...
    def test_shell_script_waits_for_pid_and_relaunches(self, tmp_path):
        ...
        zip_path = tmp_path / "update.zip"
        app_source = tmp_path / "zip_content" / "Silas.app"
        ...
        mock_app_path = tmp_path / "Applications" / "Silas.app"
        ...
        binary_path = str(mock_app_path / "Contents" / "MacOS" / "Silas")
    ...
    def test_binary_name_read_from_bundle_info_plist(self, tmp_path):
        ...
        hardcoded "Silas" — so a future bundle rename doesn't silently break
        the fallback relaunch."""
        import plistlib
        import zipfile
        from unittest.mock import patch, MagicMock

        custom_binary_name = "SilasNext"
        zip_path = tmp_path / "update.zip"
        app_source = tmp_path / "zip_content" / "Silas.app"
        ...
        # Shell-quoted; a bare 'Silas' occurrence would end with a single
        # quote, whereas 'SilasNext' does not.
        hardcoded_binary = f"{mock_app_path / 'Contents' / 'MacOS' / 'Silas'}'"
        assert hardcoded_binary not in script_content, (
            "must not fall back to hardcoded 'Silas' when the bundle reports a different name"
        )
    ...
    def test_shell_script_fallback_execs_binary_when_open_fails(self, tmp_path):
        ...
        (app_source / "Contents" / "Info.plist").write_bytes(
            plistlib.dumps({"CFBundleExecutable": "Silas"})
        )
        ...
        stub_binary = app_source / "Contents" / "MacOS" / "Silas"
        ...
        new_binary = Path(new_app_match.group(1)) / "Contents" / "MacOS" / "Silas"
    ...
    def test_uses_ditto_to_preserve_bundle_symlinks(self, tmp_path):
        ...
        "Silas.app can't be opened". The updater must extract with
    ...
    def test_ditto_extraction_failure_surfaces_as_install_failure(self, tmp_path):
        ...
        zip_path = tmp_path / "update.zip"
        app_source = tmp_path / "zip_content" / "Silas.app" / "Contents"
        app_source.mkdir(parents=True)
...
class TestInstallUpdateLinux:
    ...
    def test_shell_script_waits_for_pid(self, tmp_path):
        ...
        # Create a mock tar.gz file with Silas directory
        tar_path = tmp_path / "update.tar.gz"
        silas_dir = tmp_path / "silas_content" / "Silas"
        silas_dir.mkdir(parents=True)
        (silas_dir / "Silas").write_bytes(b"mock executable content")

        with tarfile.open(tar_path, "w:gz") as tf:
            tf.add(silas_dir, arcname="Silas")

        # Mock get_app_path to return a fake path
        mock_app_dir = tmp_path / "installed" / "Silas"
        mock_app_dir.mkdir(parents=True)
        (mock_app_dir / "Silas").write_bytes(b"old executable")
...
class TestPathEscaping:
    ...
    def test_applescript_normal_path_unchanged(self):
        path = Path('/Applications/Silas.app')
        escaped = _escape_applescript_path(path)
        assert escaped == '/Applications/Silas.app'
    ...
    def test_batch_normal_path_unchanged(self):
        path = Path('C:\\Program Files\\Silas\\Silas.exe')
        escaped = _escape_batch_path(path)
        assert escaped == 'C:\\Program Files\\Silas\\Silas.exe'
    ...
    def test_shell_normal_path_wrapped(self):
        path = Path('/opt/Silas/Silas')
        escaped = _escape_shell_path(path)
        assert escaped == "'/opt/Silas/Silas'"
