...
from silas.listening.transcript_buffer import TranscriptBuffer, TranscriptSegment
...
class TestTranscriptBuffer:
    ...
    def test_format_for_llm_with_wake_timestamp(self):
        ...
        buf = TranscriptBuffer()
        now = _now()
        buf.add("silas what time", now - 2, now)
    ...
    def test_mark_segment_processed(self):
        ...
        buf = TranscriptBuffer()
        now = _now()
        buf.add("silas whats the weather", now - 3, now - 2)
        buf.add("silas tell me a joke", now - 1, now)

        # Mark first segment as processed
        result = buf.mark_segment_processed("silas whats the weather")
        assert result is True
    ...
    def test_mark_segment_processed_case_insensitive(self):
        ...
        buf = TranscriptBuffer()
        now = _now()
        buf.add("Silas What's The Weather", now - 1, now)

        # Match with different case
        result = buf.mark_segment_processed("silas what's the weather")
        assert result is True
    ...
    def test_mark_segment_processed_strips_whitespace(self):
        ...
        buf = TranscriptBuffer()
        now = _now()
        buf.add("silas hello", now - 1, now)

        result = buf.mark_segment_processed("  silas hello  ")
        assert result is True
    ...
    def test_mark_segment_processed_marks_most_recent_match(self):
        ...
        now = _now()
        # Add same text twice
        buf.add("silas hello", now - 3, now - 2)
        ...
        segments = buf.get_all()
        # First "silas hello" (index 0) should NOT be marked
        assert segments[0].processed is False
        # "other segment" (index 1) should NOT be marked
        assert segments[1].processed is False
        # Second "silas hello" (index 2) should be marked
    ...
    def test_mark_segment_processed_skips_already_processed(self):
        ...
        buf = TranscriptBuffer()
        now = _now()
        buf.add("silas hello", now - 2, now - 1)
        buf.add("silas hello", now - 1, now)

        # Mark first call - should mark the most recent (index 1)
        result1 = buf.mark_segment_processed("silas hello")
        ...
        result2 = buf.mark_segment_processed("silas hello")
    ...
    def test_mark_segment_processed_no_match(self):
        ...
        result = buf.mark_segment_processed("silas goodbye")
        assert result is False
