...
from src.silas.memory.db import Database
...
def db_with_summaries(tmp_path):
    ...
    summaries = [
        ("2025-03-15", "User discussed work projects and deadlines.", "work,planning", "silas"),
        ("2025-01-10", "User talked about favourite coffee shops.", "food,coffee", "silas"),
        ("2025-06-22", "User mentioned upcoming holiday plans.", "travel,holiday", "silas"),
        ("2025-02-01", "User shared fitness routine details.", "health,fitness", "silas"),
    ]
...
class TestImportDiaryEndpoint:
    ...
    def setup_app(self, tmp_path):
        ...
        self.db.conn.execute(
            """INSERT INTO conversation_summaries (date_utc, ts_utc, summary, topics, source_app)
               VALUES (?, ?, ?, ?, ?)""",
            ("2025-03-15", "2025-03-15T12:00:00Z", "User likes dark roast coffee.", "food", "silas"),
        )
        self.db.conn.execute(
            """INSERT INTO conversation_summaries (date_utc, ts_utc, summary, topics, source_app)
               VALUES (?, ?, ?, ?, ?)""",
            ("2025-03-16", "2025-03-16T12:00:00Z", "User works at Acme Corp.", "work", "silas"),
        )
        self.db.conn.commit()
    ...
    @patch("src.silas.memory.graph_ops.call_llm_direct")
