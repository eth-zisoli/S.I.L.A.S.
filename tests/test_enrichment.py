...
from silas.reply.engine import (
...
class TestExtractorPromptRendering:
    ...
    def _run_and_capture_prompt(self, **kwargs) -> str:
        ...
        with patch("silas.reply.enrichment.call_llm_direct", side_effect=fake_call):
    ...
    def test_extract_returns_empty_dict_when_no_usable_response(self):
        with patch("silas.reply.enrichment.call_llm_direct", return_value=""):
...
class TestGraphEnrichmentGating:
    ...
    def _run(self, extract_return: dict, enrichment_source: str = "all"):
        ...
        with patch("silas.reply.engine.extract_search_params_for_memory", return_value=extract_return), \
...
class TestGraphContextReachesSystemMessage:
    ...
    def test_graph_context_appears_in_system_prompt(self):
        ...
        with patch(
            "silas.reply.engine.extract_search_params_for_memory",
            return_value={"keywords": ["food"], "questions": ["what cuisine does the user enjoy?"]},
        ), patch("silas.memory.graph.GraphMemoryStore", _FakeStore), \
             patch("silas.reply.engine.chat_with_messages", side_effect=fake_chat), \
             patch("silas.tools.selection.select_tools", return_value=[]):
...
class TestDigestMemoryForQuery:
    ...
    def test_empty_inputs_returns_empty(self):
        from silas.reply.enrichment import digest_memory_for_query
    ...
    def test_short_input_passes_through_unchanged(self):
        ...
        with patch("silas.reply.enrichment.call_llm_direct") as mock_llm:
            result = digest_memory_for_query(
    ...
    def test_none_sentinel_returns_empty(self):
        ...
        with patch(
            "silas.reply.enrichment.call_llm_direct",
            return_value="NONE",
        ):
            result = digest_memory_for_query(
    ...
    def test_truncates_oversized_digest(self):
        from silas.reply.enrichment import (
            _DIGEST_MAX_CHARS,
...
class TestDigestToolResultForQuery:
    ...
    def test_empty_input_returns_empty(self):
        from silas.reply.enrichment import digest_tool_result_for_query
...
class TestMaybeDigestToolResult:
    ...
    def test_llm_exception_falls_back_to_raw(self):
        ...
        raw = "payload " * 200
        with patch(
            "silas.reply.enrichment.digest_tool_result_for_query",
            side_effect=RuntimeError("boom"),
        ):
            out = _maybe_digest_tool_result(
                cfg=cfg, query="q", tool_name="webSearch", raw_tool_result=raw,
...
class TestDigestLoopForMaxTurns:
    ...
    def test_happy_path_returns_cleaned_reply_and_prompt_includes_query(self):
        from silas.reply.enrichment import digest_loop_for_max_turns
        ...
        with patch("silas.reply.enrichment.call_llm_direct",
    ...
    def test_llm_failure_returns_none(self):
        ...
        with patch(
            "silas.reply.enrichment.call_llm_direct", side_effect=boom
        ):
            out = digest_loop_for_max_turns(
                user_query="hello",
                loop_messages=[{"role": "assistant", "content": "working"}],
    ...
    def test_empty_llm_response_returns_none(self):
        ...
        with patch(
            "silas.reply.enrichment.call_llm_direct", return_value=""
        ):
            out = digest_loop_for_max_turns(
                user_query="hello",
                loop_messages=[{"role": "assistant", "content": "working"}],
