...
# Robustly locate repository root (directory containing src/silas)
_this_file = Path(__file__).resolve()
ROOT = None
for parent in _this_file.parents:
    if (parent / "src" / "silas").exists():
        ROOT = parent
        break
if ROOT is None:
...
#   ``from src.silas.x import ...``  (older style, ``src.`` prefix)
# or
#   ``from silas.x import ...``      (newer style, no prefix)
# CAUTION: those two import paths resolve to *distinct module instances*.
# A monkeypatch on ``src.silas.memory.conversation.X`` does NOT take
# effect on ``silas.memory.conversation.X`` and vice versa. When a test
...
# handlers it's ``from silas.memory.conversation import ...``), so a test
...
# Also add the src directory (optional, for backwards compatibility with direct 'silas' imports)
...
def dialogue_memory():
    ...
    from silas.memory.conversation import DialogueMemory
