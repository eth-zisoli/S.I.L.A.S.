...
Field trace (2026-05-03, gemma4:e2b):
  Turn 1 user: "how's the weather tomorrow Silas?" → no location set →
...
@patch("src.silas.memory.graph_ops.build_warm_profile",
...
def test_followup_carries_over_failed_previous_tool(
    ...
    cfg = _mock_cfg()
    dm = DialogueMemory()
    dm.add_message("user", "how's the weather tomorrow Silas")
...
@patch("src.silas.reply.engine.extract_text_from_response")
@patch("src.silas.reply.engine.chat_with_messages")
@patch("src.silas.reply.engine.run_tool_with_retries")
def test_planner_direct_exec_stamps_tool_failed(
    ...
    """
    from src.silas.tools.types import ToolExecutionResult
...
def test_walker_logs_orphan_assistant_tool_call(caplog):
    ...
    """
    from src.silas.reply.engine import _previous_turn_failed_tool_names
