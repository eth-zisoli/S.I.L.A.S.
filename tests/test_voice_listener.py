...
class TestWhisperComputeTypeFallback:
    ...
    def test_successful_load_with_int8(self):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            mock_sys.platform = "linux"
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", return_value=mock_whisper_model) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            ...
                            from silas.listening.listener import VoiceListener
    ...
    def test_fallback_from_int8_to_float16(self):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", side_effect=whisper_model_side_effect) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            mock_sd.query_devices.return_value = [{"name": "Test Mic", "max_input_channels": 1}]
                            mock_sd.InputStream.side_effect = Exception("Stop test here")
    ...
    def test_no_fallback_for_non_compute_type_errors(self):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel") as mock_class:
...
class TestWindowsCudaDetection:
    ...
    def setup_method(self):
        from silas.listening import listener
    ...
    def test_setup_nvidia_dll_path_adds_pip_package_dirs(self):
        ...
        import os
        from silas.listening.listener import _setup_nvidia_dll_path
    ...
    def test_probe_returns_cpu_and_missing_libs_when_dlls_absent(self):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener._setup_nvidia_dll_path"):
                with patch.dict("sys.modules", {"ctypes": mock_ctypes}):
                    from silas.listening.listener import _probe_windows_cuda_libraries
    ...
    def test_missing_cuda_hint_points_at_tray_action(self, capsys):
        ...
        from silas.listening.listener import _print_cuda_unavailable_hint
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", return_value=mock_whisper_model) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            with patch("silas.listening.listener._is_faster_whisper_turbo_supported", return_value=False):
                                mock_sd.query_devices.return_value = [{"name": "Test Mic", "max_input_channels": 1}]
                                mock_sd.InputStream.side_effect = Exception("Stop test here")
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", return_value=mock_whisper_model) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            with patch("silas.listening.listener._is_faster_whisper_turbo_supported", return_value=True):
                                mock_sd.query_devices.return_value = [{"name": "Test Mic", "max_input_channels": 1}]
                                mock_sd.InputStream.side_effect = Exception("Stop test here")
...
class TestRepetitiveHallucinationDetection:
    ...
    def _create_mock_listener(self):
        ...
        with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
            with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                with patch("silas.listening.listener.WhisperModel"):
                    with patch("silas.listening.listener.webrtcvad", None):
                        from silas.listening.listener import VoiceListener
...
class TestCpuOptimisations:
                ...
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", return_value=mock_whisper_model) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            mock_sd.query_devices.return_value = [{"name": "Test Mic", "max_input_channels": 1}]
                            mock_sd.InputStream.side_effect = Exception("Stop test here")
                            with patch("silas.listening.listener.os.cpu_count", return_value=8):
                                from silas.listening.listener import VoiceListener
                ...
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", return_value=mock_whisper_model) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            mock_sd.query_devices.return_value = [{"name": "Test Mic", "max_input_channels": 1}]
                            mock_sd.InputStream.side_effect = Exception("Stop test here")
                            with patch("silas.listening.listener.os.cpu_count", return_value=12):
                                from silas.listening.listener import VoiceListener
    ...
    def test_resolved_device_stored_from_ctranslate2(self):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", return_value=mock_whisper_model):
                        with patch("silas.listening.listener.sd") as mock_sd:
                            mock_sd.query_devices.return_value = [{"name": "Test Mic", "max_input_channels": 1}]
                            mock_sd.InputStream.side_effect = Exception("Stop test here")
...
class TestMicPermissionHint:
    ...
    def test_windows_hint(self):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            mock_sys.platform = "win32"
            from silas.listening.listener import _get_mic_permission_hint
                ...
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", return_value=mock_whisper_model):
                        with patch("silas.listening.listener.sd") as mock_sd:
                            ...
                            with patch("silas.listening.listener.time") as mock_time:
...
class TestResample:
    ...
    def test_identity_when_rates_match(self):
        ...
        import numpy as _np
        from silas.listening.listener import _resample
...
class TestCorruptedWhisperCacheRecovery:
    ...
    def test_corrupted_cache_parent_model_dir_deleted(self, tmp_path):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", side_effect=whisper_model_side_effect):
                        with patch("silas.listening.listener.sd") as mock_sd:
                            mock_sd.query_devices.return_value = [{"name": "Test Mic", "max_input_channels": 1}]
                            mock_sd.InputStream.side_effect = Exception("Stop test here")
                ...
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", side_effect=whisper_model_side_effect) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            ...
                            with patch("silas.listening.listener.time.sleep"):  # Skip actual sleep
                                from silas.listening.listener import VoiceListener
                ...
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel") as mock_class:
                        ...
                        with patch("silas.listening.listener.sd") as mock_sd:
                            ...
                            with patch("silas.listening.listener.time.sleep") as mock_sleep:
                                from silas.listening.listener import VoiceListener
                ...
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch("silas.listening.listener.WhisperModel", side_effect=whisper_model_side_effect) as mock_class:
                        with patch("silas.listening.listener.sd") as mock_sd:
                            ...
                            with patch("silas.listening.listener.time.sleep"):
                                from silas.listening.listener import VoiceListener
    ...
    with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
        with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
            with patch("silas.listening.listener.sd") as mock_sd:
                ...
                from silas.listening.listener import VoiceListener
                from silas.listening.intent_judge import IntentJudge, IntentJudgeConfig
...
class TestLlmWarmup:
    ...
    def test_spawns_threads_for_chat_and_distinct_judge(self):
        ...
        with patch(
            "silas.listening.listener.warm_up_ollama_model", return_value=True
        ) as chat_warm, patch(
            "silas.listening.intent_judge.warm_up_ollama_model", return_value=True
        ) as judge_warm:
    ...
    def test_deduplicates_when_chat_and_judge_share_model(self):
        ...
        with patch("silas.listening.listener.warm_up_ollama_model", return_value=True) as warm:
            threads = listener._start_llm_warmup()
            for t in threads:
                t.join(timeout=2.0)
    ...
    def test_records_failure_from_helper(self):
        ...
        with patch(
            "silas.listening.listener.warm_up_ollama_model", return_value=False
        ), patch(
            "silas.listening.intent_judge.warm_up_ollama_model", return_value=False
        ):
            threads = listener._start_llm_warmup()
            for t in threads:
                t.join(timeout=2.0)
    ...
    def test_warmup_runs_after_model_load(self):
        ...
        with patch("silas.listening.listener.sys") as mock_sys:
            ...
            with patch("silas.listening.listener.FASTER_WHISPER_AVAILABLE", True):
                with patch("silas.listening.listener.MLX_WHISPER_AVAILABLE", False):
                    with patch(
                        "silas.listening.listener.WhisperModel",
...
class TestIsWhisperHallucination:
    ...
    def test_gate_policy(self, no_speech_prob, threshold, expected):
        from silas.listening.listener import is_whisper_hallucination
    ...
    def test_mlx_and_faster_whisper_use_same_helper(self):
        ...
        """
        import inspect
        from silas.listening import listener as listener_mod
...
class TestWeatherBannerExample:
    ...
    def _make_listener(self, **cfg_overrides):
        ...
        cfg = MagicMock()
        cfg.wake_word = cfg_overrides.get("wake_word", "silas")
    ...
    def test_plain_form_when_auto_detect_enabled(self):
        ...
        with patch("silas.listening.listener.is_location_available", return_value=True):
            result = listener._weather_example("Silas")
        assert result == "\"How's the weather, Silas?\""
    ...
    def test_city_placeholder_when_location_disabled(self):
        ...
        result = listener._weather_example("Silas")
        assert result == "\"How's the weather in [your city], Silas?\""
    ...
    def test_city_placeholder_when_database_not_available(self):
        ...
        from unittest.mock import patch
        listener = self._make_listener(location_enabled=True, location_auto_detect=True)
        with patch("silas.listening.listener.is_location_available", return_value=False):
            result = listener._weather_example("Silas")
        assert result == "\"How's the weather in [your city], Silas?\""
