...
class TestPiperTTSInterface:
    ...
    def test_has_required_methods(self):
        ...
        from src.silas.output.tts import PiperTTS
...
class TestPiperTTSErrorHandling:
    ...
    def test_missing_model_with_failed_download(self, tmp_path):
        ...
        with patch("src.silas.output.tts._download_piper_voice", return_value=None):
    ...
    def test_explicit_model_path_skips_default(self, tmp_path):
        ...
        from src.silas.output.tts import PiperTTS, _get_default_piper_model_path
        from unittest.mock import patch
...
class TestPiperTTSWithMocking:
    ...
    @patch("src.silas.output.tts.os.path.exists", return_value=True)
    def test_piper_import_error_handling(self, mock_exists):
        ...
        with patch.dict("sys.modules", {"piper": None, "piper.voice": None}):
            ...
            with patch(
                "src.silas.output.tts.PiperTTS._ensure_initialized",
...
class TestPiperTTSFactory:
    ...
    def test_creates_piper_engine(self):
        ...
        from src.silas.output.tts import create_tts_engine, PiperTTS
    ...
    def test_chatterbox_engine_still_works(self):
        ...
        from src.silas.output.tts import create_tts_engine, ChatterboxTTS
...
class TestPiperTTSAutoDownload:
    ...
    def test_get_default_model_path(self):
        """Default model path should be in ~/.local/share/silas/models/piper/."""
        from src.silas.output.tts import _get_default_piper_model_path, PIPER_DEFAULT_VOICE

        path = _get_default_piper_model_path()

        assert PIPER_DEFAULT_VOICE in path
        assert path.endswith(".onnx")
        assert "silas" in path
        assert "piper" in path

    def test_get_piper_models_dir(self):
        """Models directory should be created under silas data dir."""
        from src.silas.output.tts import _get_piper_models_dir

        models_dir = _get_piper_models_dir()

        assert models_dir.exists()
        assert "silas" in str(models_dir)
    ...
    def test_default_voice_is_reasonable(self):
        ...
        from src.silas.output.tts import PIPER_DEFAULT_VOICE
...
class TestPiperTTSConfig:
    ...
    def test_tts_engine_defaults_to_piper(self):
        ...
        from src.silas.config import load_settings, get_default_config
        from unittest.mock import patch
        ...
        with patch("src.silas.config._load_json", return_value={}):
    ...
    def test_tts_engine_migrates_system_to_piper(self):
        ...
        with patch("src.silas.config._load_json", return_value=config_data):
            with patch("src.silas.config._save_json", return_value=True):
                settings = load_settings()
...
class TestPiperVoiceDownloadRetry:
    ...
    def test_429_retried_then_succeeds(self, tmp_path):
        ...
        import requests
        from src.silas.output.tts import _download_piper_voice
        ...
        with patch("requests.get", side_effect=mock_get):
            with patch("src.silas.output.tts._get_piper_models_dir", return_value=tmp_path):
                with patch("src.silas.output.tts.time.sleep") as mock_sleep:
