...
from silas.listening.state_manager import StateManager, ListeningState
from silas.listening.intent_judge import IntentJudgment
...
def _create_listener(**kwargs):
    ...
    with patch("silas.listening.listener.webrtcvad", None), \
         patch("silas.listening.listener.sd", None), \
         patch("silas.listening.listener.np", None), \
         patch("silas.listening.listener.create_intent_judge", return_value=None):
...
class TestEarlyBeepFeedback:
    ...
    def test_beep_starts_on_wake_word_before_intent_judge(self, _print):
        ...
        listener._process_transcript("silas what time is it", utterance_energy=0.01)
    ...
    def test_beep_stops_when_intent_judge_rejects(self, _print):
        ...
        listener._process_transcript("silas is a cool name", utterance_energy=0.01)

        # Query should NOT be accepted (judge rejected + fallback wake word
        # check won't find a query after "silas")
        assert not _is_beeping(listener)
        listener.state_manager.stop()
...
class TestEchoRejectionInHotWindow:
    ...
    def test_echo_rejected_before_intent_judge_can_accept(self, _print):
        ...
        The mic picks up Silas's TTS output and Whisper transcribes it.
...
class TestSpeechIgnoredOutsideHotWindow:
    ...
    def test_multiple_utterances_after_hot_window_all_ignored(self, _print):
        ...
        listener._process_transcript("silas will it rain today", utterance_energy=0.01)
...
class TestStaleWakeTimestampAcrossUtterances:
    ...
    Real-world bug: user said "Silas, remember..." (rejected by judge),
    then said "Hey Google, TV off." The judge saw the previous "Silas"
    ...
    def test_rejected_wake_utterance_does_not_vouch_for_next_utterance(self, _print):
        ...
        # First utterance: has "silas", judge rejects as not directed
        _install_intent_judge(listener, _make_judgment(
            directed=False, query="", confidence="high",
        ...
        now = time.time()
        listener._process_transcript(
            "silas i want you to remember that my other office days are thursdays",
            utterance_energy=0.01,
            utterance_start_time=now,
        ...
        # (e.g. because the earlier "silas" is still in its context buffer)
        _install_intent_judge(listener, _make_judgment(
...
class TestIntentJudgeGating:
    ...
    def test_judge_called_when_wake_word_detected(self, _print):
        ...
        listener._process_transcript(
            "silas what time is it", utterance_energy=0.01,
        )
