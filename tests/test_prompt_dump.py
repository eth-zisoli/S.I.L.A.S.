"""
Unit tests for the opt-in prompt dump (src/silas/reply/prompt_dump.py).
...
from silas.reply import prompt_dump
...
class TestGating:
    def test_disabled_by_default(self, tmp_home, monkeypatch):
        monkeypatch.delenv("SILAS_DUMP_PROMPTS", raising=False)
        ...
        # No files created.
        assert not (tmp_home / ".local" / "share" / "silas" / "prompts").exists()

    @pytest.mark.parametrize("value", ["1", "true", "YES", "on"])
    def test_enabled_by_truthy_env_values(self, tmp_home, monkeypatch, value):
        monkeypatch.setenv("SILAS_DUMP_PROMPTS", value)
...
class TestDumpContents:
    def test_writes_full_payload(self, tmp_home, monkeypatch, capsys):
        monkeypatch.setenv("SILAS_DUMP_PROMPTS", "1")
        messages = [
        ...
        assert path.exists()
        assert path.parent == tmp_home / ".local" / "share" / "silas" / "prompts"
    ...
    def test_dump_failure_is_swallowed(self, tmp_home, monkeypatch):
        ...
        with patch("silas.reply.prompt_dump.json.dumps", side_effect=RuntimeError("boom")):
            result = prompt_dump.dump_reply_turn(
                session_id="abc",
                turn=1,
                query="q",
