...
from silas.tools.selection import (
    select_tools,
...
class TestEmbeddingStrategy:
    ...
    def test_selects_similar_tools(self):
        ...
        with patch("silas.memory.embeddings.get_embedding", side_effect=mock_embed):
            result = select_tools(
                "what's the weather",
                _builtin(), {},
    ...
    def test_failed_query_embedding_falls_back(self):
        ...
        with patch("silas.memory.embeddings.get_embedding", side_effect=mock_fail):
            result = select_tools(
                "anything",
                _builtin(), _mcp(),
                strategy=ToolSelectionStrategy.EMBEDDING,
...
class TestLLMStrategy:
    ...
    def test_parses_comma_separated_response(self):
        ...
        with patch("silas.llm.call_llm_direct", side_effect=mock_llm):
            result = select_tools(
                "what's the weather",
                _builtin(), {},
    ...
    def test_caps_chatty_router_output_at_max(self):
        ...
        from silas.tools.selection import _LLM_MAX_SELECTED
