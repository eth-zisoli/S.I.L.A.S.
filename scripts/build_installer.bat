@echo off
REM Build the Windows installer (Silas-Setup-x64.exe) for manual testing.
REM PyInstaller produces dist\Silas\, then Inno Setup wraps that into the
REM installer at dist\Silas-Setup-x64.exe. The resulting installer is the
...
REM ---- Stamp a dev version file so silas.get_version() works in the bundle.
...
> "%PROJECT_ROOT%\src\silas\_version.py" (
...
REM ---- PyInstaller produces dist\Silas\.
echo [build_installer] Running PyInstaller...
"%MAMBA_ENV%\python.exe" -m PyInstaller silas_desktop.spec
if not exist "dist\Silas\Silas.exe" (
    echo [build_installer] ERROR: PyInstaller did not produce dist\Silas\Silas.exe
    exit /b 1
)
...
"%ISCC%" /DMyAppVersion="%DEV_VERSION%" installer\windows\silas_setup.iss
if errorlevel 1 (
    echo [build_installer] ERROR: Inno Setup failed
    exit /b 1
)

if not exist "dist\Silas-Setup-x64.exe" (
    echo [build_installer] ERROR: Installer was not produced at dist\Silas-Setup-x64.exe
    exit /b 1
)

echo.
echo [build_installer] SUCCESS
echo                   Installer:  %PROJECT_ROOT%\dist\Silas-Setup-x64.exe
echo                   Frozen app: %PROJECT_ROOT%\dist\Silas\Silas.exe
echo.
...
echo                   "%%LOCALAPPDATA%%\Programs\Silas\cuda\install.log".
