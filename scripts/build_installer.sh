...
# dist/Silas.app; on Linux dist/Silas/. There is no Inno-equivalent
...
# Stamp a dev version file so silas.get_version() works in the bundle.
...
cat > "${PROJECT_ROOT}/src/silas/_version.py" <<EOF
...
if [[ "$OSTYPE" == "darwin"* ]]; then
    if [[ -d dist/Silas.app ]]; then
        ...
        echo "                  Bundle: ${PROJECT_ROOT}/dist/Silas.app"
        echo "[build_installer] ℹ️  No installer is produced on macOS."
    else
        echo "[build_installer] ❌ Bundle missing at dist/Silas.app" >&2
        exit 1
    fi
else
    if [[ -d dist/Silas ]]; then
        echo
        echo "[build_installer] ✅ SUCCESS"
        echo "                  Bundle: ${PROJECT_ROOT}/dist/Silas"
        echo "[build_installer] ℹ️  No installer is produced on Linux."
    else
        echo "[build_installer] ❌ Bundle missing at dist/Silas" >&2
        exit 1
    fi
fi
