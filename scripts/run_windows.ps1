...
function Write-Info($msg) { Write-Host "[silas] $msg" }
...
if (-not $env:SILAS_VOICE_DEBUG) { $env:SILAS_VOICE_DEBUG = '0' }
...
if ($micromamba) {
  ...
  if (Test-Path $envPython) {
    ...
    micromamba run -p $envPrefix python -m silas.daemon
    exit $LASTEXITCODE
  }
}
