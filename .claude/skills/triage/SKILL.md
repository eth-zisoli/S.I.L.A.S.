...
name: triage
description: >
  Triage open GitHub issues and discussions on the Silas repo. Sweep for
...
You are triaging open issues and discussions on `isair/silas`. Work from data,
...
```bash
gh api graphql -f query='{repository(owner:"isair",name:"silas"){discussions(first:30,states:OPEN,orderBy:{field:UPDATED_AT,direction:DESC}){nodes{id number title author{login} category{name} updatedAt comments(last:5){totalCount nodes{id author{login} createdAt body replies(last:10){nodes{id author{login} createdAt body}}}}}}}}' \
...
Common Silas patterns and what they mean:
...
| `LLM connection error: ... RemoteDisconnected` | Ollama dropped. Upstream, not Silas. | `ollama run <model>` health check; Ollama version. |
| `setup_wizard.py ... _install_next_model` fatal | Real bug on our side. | Which model had just finished, which was about to start; `ollama list` after crash; `~/Library/Logs/DiagnosticReports/Silas-*.ips` on macOS. |
...
- **Windows "Error 4551: Application Control policy has blocked this file"**: WDAC / AppLocker / corporate MDM, not Silas. Point at IT allow-listing, `secpol.msc`, or install-from-source.
- **"missing AI models"**: `ollama pull gemma4:e2b` + `ollama pull nomic-embed-text`, or tray → 🔧 Setup Wizard.
- **Setup wizard was closed early, nothing works**: tray → 🔧 Setup Wizard reopens it. Fallback: `rm -rf ~/.config/silas ~/.local/share/silas/config`.
...
- **"Can Silas speak <language>?"**: yes if the chat model supports it; for voice, Whisper handles most languages. Point at README.
...
2. The mutation response returns the comment URL: `addDiscussionComment.comment.url`. Inspect it. If the host path is anything other than `github.com/isair/silas/discussions/<N>`, you posted to the wrong repo. Delete the comment immediately:
   ```bash
...
```bash
gh api -X PATCH repos/isair/silas/issues/comments/<commentId> -f body="..."
```

Discussion comment edit. First grab the comment node id (the `last:5` window usually covers recent owner replies):

```bash
gh api graphql -f query='{repository(owner:"isair",name:"silas"){discussion(number:N){comments(last:5){nodes{id author{login} createdAt body}}}}}'
```
...
comments posted, closures. Use markdown links like `[#241](https://github.com/isair/silas/issues/241)`. Keep it short.
